...
class CandidateEvaluationSerializer(serializers.ModelSerializer):
    ...
    def get_interviewer_name(self, obj):
        return (
            obj.interviewer.employee.full_name
            if obj.interviewer and obj.interviewer.employee
            else None
        )
    ...
    def get_application_details(self, obj):
        if not obj.application:
            return None
        return {
            'candidate_name': obj.application.candidate.full_name
            if obj.application.candidate
            else None,
            'job_title': obj.application.job_opening.title
            if obj.application.job_opening
            else None,
            'job_opening_id': obj.application.job_opening.id
            if obj.application.job_opening
            else None,
        }
    ...
    def get_total_score(self, obj):
        ...
        for score in obj.criterion_scores.all():
            total += float(score.score) * (
                score.criterion.weight if score.criterion else 1
            )
        return total
