...
class Account(models.Model):
    ...
    tax_rate = models.DecimalField(
        _('Taux de taxe'), max_digits=5, decimal_places=2, null=True, blank=True
    )
    ...
    def get_balance(self, start_date=None, end_date=None):
        ...
        query = JournalEntryLine.objects.filter(
            account_id=self, entry_id__state='posted'
        )
...
class JournalEntry(models.Model):
    ...
    source_module = models.CharField(
        _('Module source'), max_length=50, blank=True, null=True
    )
    source_model = models.CharField(
        _('Modèle source'), max_length=50, blank=True, null=True
    )
    ...
    def post(self):
        ...
        if self.state != 'draft':
            raise ValidationError(
                _("Impossible de valider une écriture qui n'est pas en brouillon")
            )
    ...
    def cancel(self):
        ...
        if self.state != 'posted':
            raise ValidationError(
                _("Impossible d'annuler une écriture qui n'est pas validée")
            )

        # Vérifier si l'écriture peut être annulée (pas déjà lettrée)
        if self.lines.filter(is_reconciled=True).exists():
            raise ValidationError(
                _("Impossible d'annuler une écriture dont les lignes sont lettrées")
            )
    ...
    def try_auto_reconcile(self):
        ...
        reconcilable_accounts = set(
            self.lines.filter(
                account_id__is_reconcilable=True, is_reconciled=False
        ...
        for account_id in reconcilable_accounts:
            ...
            matching_line = (
                ...
                .exclude(entry_id=self.id)
                .annotate(balance=models.F('debit') - models.F('credit'))
                .filter(balance=opposite_balance)
                .first()
            )
...
class JournalEntryLine(models.Model):
    ...
    credit = models.DecimalField(
        _('Crédit'), max_digits=15, decimal_places=2, default=0
    )
    ...
    amount_currency = models.DecimalField(
        _('Montant en devise'), max_digits=15, decimal_places=2, default=0
    )
    ...
    # Analytique
    analytic_account_id = models.ForeignKey(
        'AnalyticAccount',
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    ...
    tax_base_amount = models.DecimalField(
        _('Base de taxe'), max_digits=15, decimal_places=2, default=0
    )
    ...
    def clean(self):
        ...
        if not self.debit and not self.credit:
            raise ValidationError(
                _('Une ligne doit avoir soit un débit soit un crédit')
            )
...
class BankStatement(models.Model):
    ...
    balance_start = models.DecimalField(
        _('Solde initial'), max_digits=15, decimal_places=2
    )
    balance_end = models.DecimalField(_('Solde final'), max_digits=15, decimal_places=2)
    balance_end_real = models.DecimalField(
        _('Solde final réel'), max_digits=15, decimal_places=2, null=True, blank=True
    )
    state = models.CharField(
        _('État'),
        max_length=20,
...
class BankStatementLine(models.Model):
    ...
    journal_entry_line_ids = models.ManyToManyField(
        JournalEntryLine,
        blank=True,
        related_name='statement_lines',
...
class Asset(models.Model):
    ...
    acquisition_value = models.DecimalField(
        _("Valeur d'acquisition"), max_digits=15, decimal_places=2
    )
    salvage_value = models.DecimalField(
        _('Valeur résiduelle'), max_digits=15, decimal_places=2, default=0
    )
    ...
    first_depreciation_date = models.DateField(
        _("Date de début d'amortissement"), null=True, blank=True
    )

    # Écriture d'acquisition
    acquisition_move_id = models.ForeignKey(
        JournalEntry,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    ...
    def compute_depreciation_board(self):
        ...
        if self.state not in ['draft', 'open']:
            raise ValidationError(
                _(
                    "Impossible de calculer l'amortissement pour une immobilisation clôturée ou cédée"
                )
            )
...
class AssetDepreciation(models.Model):
    ...
    remaining_value = models.DecimalField(
        _('Valeur restante'), max_digits=15, decimal_places=2
    )
