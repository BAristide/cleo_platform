...
from .serializers import (
    PayrollPeriodSerializer, PayrollParameterSerializer, 
    ContractTypeSerializer, SalaryComponentSerializer,
    TaxBracketSerializer, EmployeePayrollSerializer,
    PayrollRunSerializer, PaySlipSerializer,
    PaySlipLineSerializer, AdvanceSalarySerializer
)
...
class PayrollPeriodViewSet(viewsets.ModelViewSet):
    ...
    def close_period(self, request, pk=None):
        ...
        open_runs = PayrollRun.objects.filter(
            period=period,
            status__in=['draft', 'in_progress', 'calculated']
        )
...
class EmployeePayrollViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['employee__first_name', 'employee__last_name', 'employee__employee_id', 'cnss_number', 'bank_account']
...
class PaySlipViewSet(viewsets.ModelViewSet):
    ...
    ordering_fields = ['payroll_run__period__start_date', 'employee__last_name', 'number']
    ...
    def download_pdf(self, request, pk=None):
        ...
        if os.path.exists(pdf_path):
            ...
            response['Content-Disposition'] = f'attachment; filename="bulletin_{payslip.number}.pdf"'
            return response
        else:
            return Response(
...
class AdvanceSalaryViewSet(viewsets.ModelViewSet):
    ...
    def mark_as_paid(self, request, pk=None):
        ...
        return Response({
            "success": True,
            "message": "Acompte marqué comme payé avec succès"
        })
...
def payroll_dashboard(request):
    ...
    # Période en cours
    current_period = PayrollPeriod.objects.filter(is_closed=False).order_by('-start_date').first()
    
    # Statistiques sur les lancements de paie
    payroll_runs = PayrollRun.objects.all()
    runs_by_status = payroll_runs.values('status').annotate(count=Count('id')).order_by('status')
    
    # Statistiques sur les bulletins
    payslips = PaySlip.objects.all()
    payslips_by_status = payslips.values('status').annotate(count=Count('id')).order_by('status')
    
    # Statistiques sur les acomptes
    advances = AdvanceSalary.objects.all()
    advances_by_period = advances.values('period__name').annotate(
        count=Count('id'),
        total=Sum('amount')
    ...
    if current_period:
        current_period_data = {
            ...
            'payslips_count': PaySlip.objects.filter(payroll_run__period=current_period).count(),
    ...
    if current_period:
        department_stats = PaySlip.objects.filter(
            payroll_run__period=current_period
        ).values(
