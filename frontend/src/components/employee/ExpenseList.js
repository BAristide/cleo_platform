// src/components/employee/ExpenseList.js
...
const ExpenseList = () => {
    ...
    return (
      <Space size="small">
        {isMine && record.status === 'draft' && (
          <>
            <Button size="small" onClick={() => navigate(`/my-space/expenses/${record.id}/edit`)}>
              Modifier
            </Button>
        ...
        {isMine && ['draft', 'submitted'].includes(record.status) && (
          <Button size="small" danger onClick={() => openAction('cancel', record)}>Annuler</Button>
        )}
        {isManager && record.status === 'submitted' && (
          <>
            <Button size="small" type="primary" icon={<CheckOutlined />}
              onClick={() => openAction('approve_manager', record)}>Approuver</Button>
            <Button size="small" danger icon={<CloseOutlined />}
              onClick={() => openAction('reject', record)}>Rejeter</Button>
          </>
        )}
        {isFinance && record.status === 'approved_manager' && (
          <>
            <Button size="small" type="primary" icon={<CheckOutlined />}
              onClick={() => openAction('approve_finance', record)}>Approuver</Button>
            <Button size="small" danger icon={<CloseOutlined />}
              onClick={() => openAction('reject', record)}>Rejeter</Button>
          </>
        )}
        {isFinance && record.status === 'approved_finance' && (
          <Button size="small" type="primary" onClick={() => openAction('reimburse', record)}>Rembourser</Button>
        )}
        {['approved_finance', 'reimbursed'].includes(record.status) && (
          <Button size="small" icon={<DownloadOutlined />}
            onClick={() => window.open(`/api/hr/expense-reports/${record.id}/download_pdf/`, '_blank')}>PDF</Button>
        )}
      </Space>
    );
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: s => { const c = STATUS_CONFIG[s] || { label: s, color: 'default' }; return <Tag color={c.color}>{c.label}</Tag>; },
    },
    { title: 'Actions', key: 'actions', render: (_, r) => renderActions(r) },
  ];

  const actionLabels = {
    submit: 'Soumettre la note', approve_manager: 'Approbation N+1',
    approve_finance: 'Approbation Finance', reimburse: 'Marquer comme remboursée',
    reject: 'Rejeter la note', cancel: 'Annuler la note',
  };
  ...
  const tableProps = { columns, rowKey: 'id', loading, size: 'small', pagination: { pageSize: 15 } };
    ...
    <div>
      <Row justify="space-between" align="middle" style={{ marginBottom: 16 }}>
        ...
        <Col>
          <Space>
            ...
            <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/my-space/expenses/new')}>
              Nouvelle note
            </Button>
          </Space>
        </Col>
      ...
      {(isManager || isFinance) ? (
        <Tabs defaultActiveKey="mine">
          ...
          <TabPane tab={`Approbations en attente${pendingApprovals.length > 0 ? ` (${pendingApprovals.length})` : ''}`} key="pending">
      ...
      <Modal title={actionLabels[actionModal.action] || ''} open={actionModal.open}
        onOk={handleAction} onCancel={() => { setActionModal({ open: false, action: null, record: null }); setNotes(''); }}
        okText="Confirmer" cancelText="Annuler">
        {needsNotes && (
          <Input.TextArea rows={3} placeholder={actionModal.action === 'reject' ? 'Motif de rejet (obligatoire)' : 'Notes (optionnel)'}
