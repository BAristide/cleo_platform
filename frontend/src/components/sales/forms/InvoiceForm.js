...
const InvoiceForm = () => {
  ...
  const fetchInvoiceDetails = async () => {
    ...
    try {
      ...
      setInvoiceType(invoiceData.type || 'standard');

      // Récupérer les contacts de l'entreprise
      if (invoiceData.company) {
        await fetchContactsByCompany(invoiceData.company);
        setSelectedCompany(invoiceData.company);
      }
      ...
      // Mise à jour de l'état d'exonération de TVA
      setIsExempt(invoiceData.is_tax_exempt || false);

      // Mise à jour des références
      if (invoiceData.quote) {
        setSelectedQuote(invoiceData.quote);
      }
      if (invoiceData.order) {
        setSelectedOrder(invoiceData.order);
      }
      ...
      const itemsResponse = await axios.get(`/api/sales/invoice-items/?invoice=${id}`);
  ...
  const handleCurrencyChange = (value) => {
    ...
    calculateTotals(invoiceItems, form.getFieldValue('discount_percentage') || 0, isForeignCurrency);
  };
  ...
  const handleExemptChange = (checked) => {
    ...
    // Recalculer les totaux
    calculateTotals(invoiceItems, form.getFieldValue('discount_percentage') || 0, checked);
  };

  const handleDiscountChange = (value) => {
    calculateTotals(invoiceItems, value || 0, isExempt);
  };

  const handlePaymentTermsChange = (value) => {
    // Calculer la date d'échéance à partir de la date de facture et des conditions de paiement
    const invoiceDate = form.getFieldValue('date');
    if (invoiceDate) {
      let dueDate;
      switch (value) {
        case 'immediate':
          dueDate = moment(invoiceDate); // Même jour
          break;
        case '30_days':
          dueDate = moment(invoiceDate).add(30, 'days');
          break;
        case '60_days':
          dueDate = moment(invoiceDate).add(60, 'days');
          break;
        default:
          dueDate = moment(invoiceDate).add(30, 'days'); // Par défaut 30 jours
      }
  ...
  const handleDateChange = (date) => {
    // Mettre à jour la date d'échéance en fonction de la nouvelle date de facture
    if (date) {
      const paymentTerms = form.getFieldValue('payment_terms') || '30_days';
  ...
  const handleQuoteSelect = async (quoteId) => {
    ...
    try {
      ...
      handleCompanyChange(quoteData.company);
      handleCurrencyChange(quoteData.currency);
      setIsExempt(quoteData.is_tax_exempt || false);

      // Récupérer les éléments du devis
      const itemsResponse = await axios.get(`/api/sales/quote-items/?quote=${quoteId}`);
      const quoteItems = extractResultsFromResponse(itemsResponse);

      // Convertir les éléments du devis en éléments de facture
      ...
      calculateTotals(newInvoiceItems, quoteData.discount_percentage || 0, quoteData.is_tax_exempt || false);
    } catch (error) {
  ...
  const handleOrderSelect = async (orderId) => {
    ...
    try {
      ...
      form.setFieldsValue({
        ...
        is_tax_exempt: orderData.is_tax_exempt || false,
        tax_exemption_reason: orderData.tax_exemption_reason,
        notes: orderData.notes,
        terms: orderData.terms,
        quote: orderData.quote // Si la commande provient d'un devis
      });
      ...
      // Mettre à jour les états
      handleCompanyChange(orderData.company);
      handleCurrencyChange(orderData.currency);
      setIsExempt(orderData.is_tax_exempt || false);

      // Récupérer les éléments de la commande
      const itemsResponse = await axios.get(`/api/sales/order-items/?order=${orderId}`);
      const orderItems = extractResultsFromResponse(itemsResponse);

      // Convertir les éléments de la commande en éléments de facture
      const newInvoiceItems = orderItems.map(item => ({
        id: `temp_${Date.now()}_${item.id}`,
        product: item.product,
        product_name: item.product_name,
        product_reference: item.product_reference,
      ...
      setInvoiceItems(newInvoiceItems);
      calculateTotals(newInvoiceItems, orderData.discount_percentage || 0, orderData.is_tax_exempt || false);
    } catch (error) {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (isEditMode) {
        await axios.put(`/api/sales/invoices/${id}/`, formData);
        invoiceId = id;
        message.success("Facture mise à jour avec succès");
      } else {
        const response = await axios.post('/api/sales/invoices/', formData);
      ...
      for (const item of invoiceItems) {
        ...
        if (item.id && item.id.toString().startsWith('temp_')) {
          // Nouvel élément
          await axios.post('/api/sales/invoice-items/', itemData);
        } else {
          // Élément existant à mettre à jour
          await axios.put(`/api/sales/invoice-items/${item.id}/`, itemData);
        }
      }
  ...
  return (
    <Card title={isEditMode ? "Modifier la facture" : "Nouvelle facture"}>
      <Button
      ...
      >
        ...
        >
          ...
          >
            <Radio value="standard">Facture standard</Radio>
            <Radio value="deposit">Facture d'acompte</Radio>
          </RadioGroup>
        </Form.Item>

        {/* Références aux devis et commandes */}
        <Row gutter={16}>
          <Col span={12}>
            <Form.Item
              name="quote"
            ...
            >
              <Select
                ...
                disabled={isEditMode} // Ne peut pas changer le devis en mode édition
              >
                {quoteOptions.map(quote => (
                  <Option key={quote.id} value={quote.id}>{quote.number} - {quote.company_name}</Option>
                ))}
              </Select>
            </Form.Item>
          </Col>
          <Col span={12}>
            ...
            >
              <Select
                ...
                disabled={isEditMode} // Ne peut pas changer la commande en mode édition
              >
                {orderOptions.map(order => (
                  <Option key={order.id} value={order.id}>{order.number} - {order.company_name}</Option>
                ))}
              </Select>
            </Form.Item>
          </Col>
          ...
          <Col span={12}>
            ...
            >
              <Select
                ...
                disabled={!selectedCompany || ((selectedQuote || selectedOrder) && !isEditMode)}
        ...
        {isExempt && (
          <Form.Item
            ...
            style={{ marginTop: 16 }}
            rules={[
              { required: isExempt, message: 'Veuillez indiquer la raison de l\'exonération de TVA' }
            ]}
          >
            <Input />
          </Form.Item>
        ...
        <Row gutter={16}>
          ...
          <Col span={12}>
            <Form.Item
              ...
              label="Conditions"
            >
              <TextArea rows={4} placeholder="Conditions spécifiques à cette facture" />
            </Form.Item>
          </Col>
        </Row>
