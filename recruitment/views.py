...
from .serializers import (
    JobOpeningSerializer, CandidateSerializer, ApplicationSerializer,
    InterviewPanelSerializer, InterviewerSerializer, EvaluationCriterionSerializer,
    CandidateEvaluationSerializer, CriterionScoreSerializer,
    RecruitmentStatsSerializer, RecruitmentNotificationSerializer,
    ApplicationCreateSerializer, JobOpeningDetailSerializer
)
...
class JobOpeningViewSet(viewsets.ModelViewSet):
    ...
    def publish(self, request, pk=None):
        ...
        if job_opening.status != 'draft':
            return Response({'error': 'Seuls les postes en brouillon peuvent être publiés.'},
    ...
    def close(self, request, pk=None):
        ...
        if job_opening.status == 'closed':
            return Response({'error': 'Cette offre est déjà clôturée.'},
                           status=status.HTTP_400_BAD_REQUEST)
...
class ApplicationViewSet(viewsets.ModelViewSet):
    ...
    def preselect(self, request, pk=None):
        ...
        if application.status != 'received':
            return Response({'error': 'Seules les candidatures reçues peuvent être présélectionnées.'},
                           status=status.HTTP_400_BAD_REQUEST)
    ...
    def reject(self, request, pk=None):
        ...
        if application.status in ['selected', 'hired', 'withdrawn']:
            return Response({'error': 'Cette candidature ne peut pas être rejetée.'},
                           status=status.HTTP_400_BAD_REQUEST)
    ...
    def schedule_interview(self, request, pk=None):
        ...
        if application.status not in ['preselected', 'analysis']:
            return Response({'error': 'Cette candidature ne peut pas être programmée pour un entretien.'},
                           status=status.HTTP_400_BAD_REQUEST)
        ...
        if not interview_date:
            return Response({'error': 'La date d\'entretien est requise.'},
                           status=status.HTTP_400_BAD_REQUEST)
    ...
    def mark_interviewed(self, request, pk=None):
        ...
        if application.status != 'selected_for_interview':
            return Response({'error': 'Seules les candidatures programmées pour un entretien peuvent être marquées comme interviewées.'},
                           status=status.HTTP_400_BAD_REQUEST)
    ...
    def select(self, request, pk=None):
        ...
        if application.status != 'interviewed':
            return Response({'error': 'Seules les candidatures interviewées peuvent être sélectionnées.'},
                           status=status.HTTP_400_BAD_REQUEST)
    ...
    def hire(self, request, pk=None):
        ...
        if application.status != 'selected':
            return Response({'error': 'Seules les candidatures sélectionnées peuvent être embauchées.'},
                           status=status.HTTP_400_BAD_REQUEST)
...
class InterviewPanelViewSet(viewsets.ModelViewSet):
    ...
    def notify_interviewers(self, request, pk=None):
        ...
        if not all([interview_date, interview_location, candidate_name, job_title]):
            return Response({'error': 'Données d\'entretien incomplètes.'},
                           status=status.HTTP_400_BAD_REQUEST)
        ...
        for interviewer in panel.interviewers.all():
            ...
            if hasattr(settings, 'EMAIL_HOST_USER') and settings.EMAIL_HOST_USER:
                ...
                html_message = render_to_string('recruitment/emails/interview_notification.html', context)
...
class CandidateEvaluationViewSet(viewsets.ModelViewSet):
    ...
    def my_evaluations(self, request):
        ...
        try:
            ...
            return Response({'error': 'Aucun employé associé à cet utilisateur.'},
                           status=status.HTTP_400_BAD_REQUEST)
        
        # Récupérer les instances d'Interviewer pour cet employé
        interviewer_ids = Interviewer.objects.filter(employee=employee).values_list('id', flat=True)
        
        # Filtrer les évaluations
        evaluations = CandidateEvaluation.objects.filter(interviewer_id__in=interviewer_ids).order_by('-evaluation_date')
...
class RecruitmentStatsViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def dashboard(self, request):
        ...
        openings_by_department = JobOpening.objects.values(
            'department__name'
        ).annotate(
            count=Count('id')
        ...
        recent_hires = Application.objects.filter(
            status='hired'
        ...
        response_data = {
            'general': {
                ...
                'applications_per_opening': round(total_applications / total_openings if total_openings else 0, 2)
            },
            'applications_by_status': applications_by_status,
            'openings_by_department': openings_by_department,
            'recent_hires': ApplicationSerializer(recent_hires, many=True).data,
            'upcoming_interviews': ApplicationSerializer(upcoming_interviews, many=True).data
        }
    ...
    def generate(self, request):
        ...
        if not period_start or not period_end:
            return Response({'error': 'Les dates de début et de fin sont requises.'},
                           status=status.HTTP_400_BAD_REQUEST)
        
        # Calculer les statistiques
        total_job_openings = JobOpening.objects.filter(
            opening_date__gte=period_start,
        ...
        preselected_applications = Application.objects.filter(
            status__in=['preselected', 'analysis', 'selected_for_interview', 'interviewed', 'selected', 'hired'],
            application_date__gte=period_start,
            application_date__lte=period_end
        ).count()
        ...
        # Calculer les taux
        preselection_rate = round((preselected_applications / total_applications) * 100, 2) if total_applications else 0
        interview_rate = round((interviewed_candidates / preselected_applications) * 100, 2) if preselected_applications else 0
        hiring_rate = round((hired_candidates / interviewed_candidates) * 100, 2) if interviewed_candidates else 0
        
        # Calculer le temps moyen d'embauche
        avg_time = Application.objects.filter(
            status='hired',
            application_date__gte=period_start,
            application_date__lte=period_end
        ).annotate(
            days_to_hire=F('updated_at') - F('application_date')
        ).aggregate(
        ...
        stats, created = RecruitmentStats.objects.update_or_create(
            ...
            defaults={
                ...
                'total_applications': total_applications,
                'applications_per_opening': round(total_applications / total_job_openings, 2) if total_job_openings else 0,
...
class RecruitmentNotificationViewSet(viewsets.ModelViewSet):
    ...
    def my_notifications(self, request):
        ...
        notifications = RecruitmentNotification.objects.filter(recipient=employee).order_by('-created_at')
    ...
    def mark_all_as_read(self, request):
        ...
        return Response({'status': f'{updated} notification(s) marquée(s) comme lue(s)'})
