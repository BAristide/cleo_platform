...
from ..models import (
    JournalEntry, JournalEntryLine, Account, FiscalPeriod, Journal,
    FiscalYear, Tax
)
...
class ImportExportService:
    ...
    def export_journal_entries(start_date, end_date, journal_id=None, format='csv'):
        ...
        lines = JournalEntryLine.objects.filter(
            entry_id__in=entries
        ).order_by('entry_id__date', 'entry_id__id', 'id')
    ...
    def _export_to_csv(lines):
        ...
        # En-têtes
        headers = [
            _('Date'), _('Journal'), _('Numéro pièce'), _('Référence'),
            _('Compte'), _('Libellé'), _('Partenaire'),
            _('Débit'), _('Crédit'), _('Analytique')
        ]
    ...
    def _export_to_excel(lines):
        ...
        for row, line in enumerate(lines, 1):
            worksheet.write_datetime(row, 0, datetime.combine(line.entry_id.date, datetime.min.time()), date_format)
            ...
            worksheet.write(row, 9, line.analytic_account_id.code if line.analytic_account_id else '')
    ...
    def _export_to_xml(lines):
        ...
        for entry_data in entries_dict.values():
            ...
            for line in entry_data['lines']:
                ...
                etree.SubElement(line_element, "AccountCode").text = line.account_id.code
                etree.SubElement(line_element, "Label").text = line.name
                etree.SubElement(line_element, "Partner").text = line.partner_id.name if line.partner_id else ''
                etree.SubElement(line_element, "Debit").text = str(line.debit)
                etree.SubElement(line_element, "Credit").text = str(line.credit)
                
                if line.analytic_account_id:
                    etree.SubElement(line_element, "AnalyticCode").text = line.analytic_account_id.code
                
                if line.date_maturity:
                    etree.SubElement(line_element, "DueDate").text = line.date_maturity.isoformat()
    ...
    def _import_from_csv(file_data):
        ...
        required_columns = [_('Date'), _('Journal'), _('Compte'), _('Libellé'), _('Débit'), _('Crédit')]
        ...
        if missing_columns:
            return {
                ...
                'errors': [{
                    ...
                    'message': _("Colonnes manquantes dans le fichier CSV: {}").format(', '.join(missing_columns))
                }]
            }
        ...
        for row_num, row in enumerate(reader, 2):  # 2 car la première ligne est l'en-tête
            try:
                ...
                try:
                    ...
                    raise ValueError(_("Format de débit invalide: {}").format(debit_str))
                
                try:
                    credit = float(credit_str.replace(',', '.')) if credit_str else 0
                except ValueError:
                    raise ValueError(_("Format de crédit invalide: {}").format(credit_str))
                ...
                if entry_key not in entries:
                    ...
                    try:
                        ...
                        raise ValueError(_("Période fiscale non trouvée pour la date: {}").format(date_value))
        ...
        for entry_key, entry_data in entries.items():
            ...
            if abs(total_debit - total_credit) > Decimal('0.01'):  # Tolérance pour les erreurs d'arrondi
                errors.append({
                    'entry': entry_key,
                    'type': 'unbalanced',
                    'message': _("L'écriture n'est pas équilibrée (débit: {}, crédit: {})").format(total_debit, total_credit)
                })
        ...
        with transaction.atomic():
            for entry_key, entry_data in entries.items():
                ...
                entry = JournalEntry.objects.create(
                    ...
                    is_manual=True,
                    created_by=User.objects.filter(is_superuser=True).first()  # À adapter selon l'utilisateur
                )
                ...
                for line_data in entry_data['lines']:
                    ...
                    if line_data['partner_code']:
                        ...
                        try:
                            partner = Company.objects.get(code=line_data['partner_code'])
                    ...
                    if line_data['analytic_code']:
                        try:
                            ...
                            try:
                                analytic_account = AnalyticAccount.objects.get(code=line_data['analytic_code'])
        ...
        return {
            'success': True,
            'message': _("{} écritures importées avec succès").format(len(created_entries)),
            'entries': [{'id': entry.id, 'name': entry.name} for entry in created_entries]
        }
        ...
        for entry_index, entry_elem in enumerate(root.xpath('//JournalEntry')):
            try:
                ...
                for line_elem in entry_elem.xpath('.//Line'):
                    ...
                    if account_code_elem is None or debit_elem is None or credit_elem is None:
                        raise ValueError(_("Éléments AccountCode, Debit ou Credit manquants"))
    ...
    def export_ledger_to_excel(ledger_data):
        ...
        worksheet.merge_range('A2:I2', _('Période du {} au {}').format(
        ...
        # En-têtes
        headers = [
            _('Date'), _('Journal'), _('Pièce'), _('Référence'),
            _('Partenaire'), _('Libellé'), _('Débit'), _('Crédit'), _('Solde')
        ]
        ...
        for account_data in ledger_data['ledger']:
            ...
            # Titre du compte
            worksheet.merge_range(f'A{row}:I{row}', f"{account['code']} - {account['name']}", bold_format)
            row += 1
            ...
            for entry in account_data['entries']:
                worksheet.write_datetime(row-1, 0, datetime.combine(entry['date'], datetime.min.time()), date_format)
    ...
    def export_ledger_to_pdf(ledger_data):
        ...
        # Rendre le HTML
        html_string = render_to_string('accounting/reports/general_ledger.html', context)
    ...
    def export_balance_to_excel(balance_data):
        ...
        worksheet.merge_range('A2:F2', _('Au {}').format(
            balance_data['date'].strftime('%d/%m/%Y')
        ), bold_format)
        
        # En-têtes
        headers = [
            _('Code'), _('Compte'), _('Débit'), _('Crédit'),
            _('Solde débiteur'), _('Solde créditeur')
        ]
        ...
        worksheet.write_number(row, 4, balance_data['total_debit_balance'], number_format)
        worksheet.write_number(row, 5, balance_data['total_credit_balance'], number_format)
    ...
    def export_balance_sheet_to_excel(balance_sheet_data):
        ...
        worksheet.merge_range('A2:C2', _('Au {}').format(
            balance_sheet_data['date'].strftime('%d/%m/%Y')
        ), bold_format)
        ...
        worksheet.write_number(row_actif, 2, balance_sheet_data['total_assets'], number_format)
        row_actif += 2
        ...
        worksheet.write_number(row_actif, 2, balance_sheet_data['total_liabilities'], number_format)
    ...
    def export_income_statement_to_excel(income_statement_data):
        ...
        worksheet.merge_range('A2:C2', _('Période du {} au {}').format(
        ...
        worksheet.write_number(row, 2, income_statement_data['total_expenses'], number_format)
        row += 2
        ...
        worksheet.write_number(row, 2, income_statement_data['total_incomes'], number_format)
        row += 2
    ...
    def export_income_statement_to_pdf(income_statement_data):
        ...
        # Rendre le HTML
        html_string = render_to_string('accounting/reports/income_statement.html', context)
    ...
    def export_vat_declaration_to_excel(vat_data):
        ...
        worksheet.merge_range('A2:C2', _('Période: {}').format(
            vat_data['period']['name']
        ), bold_format)
        ...
        worksheet.write_number(row, 2, vat_data['vat_collected']['total'], number_format)
        row += 2
        ...
        worksheet.write_number(row, 2, vat_data['vat_deductible']['total'], number_format)
        row += 2
    ...
    def export_vat_declaration_to_pdf(vat_data):
        ...
        # Rendre le HTML
        html_string = render_to_string('accounting/reports/vat_declaration.html', context)
