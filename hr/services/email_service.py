...
class EmailService:
    ...
    @staticmethod
    def send_mission_notification(mission, to_employee=True, to_manager=False, to_hr=False):
        """
        ...
        # Ajouter le manager
        if to_manager and mission.employee and mission.employee.manager and mission.employee.manager.email:
            recipients.append(mission.employee.manager.email)
        
        # Ajouter les RH (tous les employés avec le rôle RH)
        if to_hr:
            from ..models import Employee
            hr_emails = Employee.objects.filter(is_hr=True, is_active=True).values_list('email', flat=True)
    ...
    @staticmethod
    def send_training_plan_notification(training_plan, to_employee=True, to_manager=False, to_hr=False):
        """
        ...
        # Ajouter le manager
        if to_manager and training_plan.employee and training_plan.employee.manager and training_plan.employee.manager.email:
