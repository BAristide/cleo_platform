...
class AccountAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {'fields': ('code', 'name', 'type_id', 'parent_id', 'description')}),
        (_('Attributs'), {'fields': ('is_reconcilable', 'is_active')}),
        (
            _('TVA'),
            {
...
class JournalEntryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {'fields': ('journal_id', 'name', 'date', 'period_id', 'ref', 'state')}),
        (_('Informations'), {'fields': ('narration', 'is_manual')}),
        (
            _('Origine'),
            {
    ...
    def action_post(self, request, queryset):
        ...
        for entry in queryset.filter(state='draft'):
            try:
                ...
                self.message_user(
                    request,
                    f"Erreur lors de la validation de l'écriture {entry.name}: {str(e)}",
                    level='ERROR',
                )
                errors += 1
        ...
        if count:
            self.message_user(
                request,
                _('%(count)d écriture(s) validée(s) avec succès.') % {'count': count},
            )
        if errors:
            self.message_user(
                request,
                _("%(count)d écriture(s) n'ont pas pu être validées.")
                % {'count': errors},
                level='WARNING',
            )
    ...
    def action_cancel(self, request, queryset):
        ...
        for entry in queryset.filter(state='posted'):
            try:
                ...
                self.message_user(
                    request,
                    f"Erreur lors de l'annulation de l'écriture {entry.name}: {str(e)}",
                    level='ERROR',
                )
                errors += 1
        ...
        if count:
            self.message_user(
                request,
                _('%(count)d écriture(s) annulée(s) avec succès.') % {'count': count},
            )
        if errors:
            self.message_user(
                request,
                _("%(count)d écriture(s) n'ont pas pu être annulées.")
                % {'count': errors},
                level='WARNING',
            )
...
class JournalEntryLineAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {'fields': ('entry_id', 'account_id', 'name', 'partner_id')}),
        (_('Montants'), {'fields': ('debit', 'credit')}),
        (
            _('Devises'),
            {
    ...
    def action_reconcile(self, request, queryset):
        ...
        if queryset.filter(is_reconciled=True).exists():
            self.message_user(
                request, _('Certaines lignes sont déjà lettrées.'), level='ERROR'
            )
            return
        ...
        if len(account_ids) != 1:
            self.message_user(
                request,
                _('Les lignes à lettrer doivent concerner le même compte.'),
                level='ERROR',
            )
            return
        ...
        if not account.is_reconcilable:
            self.message_user(
                request, _("Le compte sélectionné n'est pas lettrable."), level='ERROR'
            )
            return
        ...
        if abs(balance) > 0.01:  # Tolérance pour les erreurs d'arrondi
            self.message_user(
                request,
                _(
                    'Les lignes sélectionnées ne sont pas équilibrées (différence de %(balance).2f).'
                )
                % {'balance': balance},
                level='ERROR',
            )
        ...
        self.message_user(
            request,
            _('%(count)d lignes ont été lettrées avec succès.')
...
class ReconciliationAdmin(admin.ModelAdmin):
    ...
    fieldsets = ((None, {'fields': ('name', 'date', 'account_id', 'created_by')}),)
...
class FiscalYearAdmin(admin.ModelAdmin):
    ...
    fieldsets = ((None, {'fields': ('name', 'start_date', 'end_date', 'state')}),)
    ...
    def action_open(self, request, queryset):
        ...
        if count:
            self.message_user(
                request,
                _('%(count)d exercice(s) ouvert(s) avec succès.') % {'count': count},
            )
    ...
    def action_close(self, request, queryset):
        ...
        for year in queryset.filter(state='open'):
            ...
            if year.periods.filter(state='open').exists():
                self.message_user(
                    request,
                    _(f"L'exercice {year.name} a des périodes encore ouvertes."),
                    level='ERROR',
                )
                errors += 1
                continue
        ...
        if count:
            self.message_user(
                request,
                _('%(count)d exercice(s) clôturé(s) avec succès.') % {'count': count},
            )
        if errors:
            self.message_user(
                request,
                _("%(count)d exercice(s) n'ont pas pu être clôturés.")
                % {'count': errors},
                level='WARNING',
            )
    ...
    def action_create_periods(self, request, queryset):
        ...
        if count:
            self.message_user(
                request,
                _('Périodes créées pour %(count)d exercice(s).') % {'count': count},
            )
...
class FiscalPeriodAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {'fields': ('fiscal_year', 'name', 'start_date', 'end_date', 'state')}),
    )
    ...
    def action_open(self, request, queryset):
        ...
        for period in queryset.filter(state='draft'):
            ...
            if period.fiscal_year.state != 'open':
                self.message_user(
                    request,
                    _(f'La période {period.name} appartient à un exercice non ouvert.'),
                    level='ERROR',
                )
                errors += 1
                continue
        ...
        if count:
            self.message_user(
                request,
                _('%(count)d période(s) ouverte(s) avec succès.') % {'count': count},
            )
        if errors:
            self.message_user(
                request,
                _("%(count)d période(s) n'ont pas pu être ouvertes.")
                % {'count': errors},
                level='WARNING',
            )
    ...
    def action_close(self, request, queryset):
        ...
        if count:
            self.message_user(
                request,
                _('%(count)d période(s) clôturée(s) avec succès.') % {'count': count},
            )
...
class AnalyticAccountAdmin(admin.ModelAdmin):
    ...
    fieldsets = ((None, {'fields': ('code', 'name', 'parent_id', 'active')}),)
...
class TaxAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {'fields': ('name', 'description', 'amount', 'type', 'active')}),
        (
            _('Comptabilité'),
...
class BankStatementAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {'fields': ('journal_id', 'name', 'date', 'reference', 'state')}),
        (_('Soldes'), {'fields': ('balance_start', 'balance_end', 'balance_end_real')}),
        (
            _('Métadonnées'),
            {
                'fields': ('created_by', 'created_at', 'updated_at'),
    ...
    def action_confirm(self, request, queryset):
        ...
        if count:
            self.message_user(
                request,
                _('%(count)d relevé(s) confirmé(s) avec succès.') % {'count': count},
            )
...
class AssetAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {'fields': ('code', 'name', 'category_id', 'state')}),
        (
            _('Acquisition'),
            {
                'fields': (
        ...
        (
            _('Métadonnées'),
            {'classes': ('collapse',), 'fields': ('note', 'created_at', 'updated_at')},
        ),
    )
    ...
    def action_compute_depreciation(self, request, queryset):
        ...
        if count:
            self.message_user(
                request,
                _("Tableau d'amortissement calculé pour %(count)d immobilisation(s).")
                % {'count': count},
            )
    ...
    def action_open(self, request, queryset):
        ...
        if count:
            self.message_user(
                request,
                _('%(count)d immobilisation(s) mise(s) en service.') % {'count': count},
            )

    action_open.short_description = _(
        'Mettre en service les immobilisations sélectionnées'
    )
    ...
    def action_close(self, request, queryset):
        ...
        if count:
            self.message_user(
                request,
                _('%(count)d immobilisation(s) clôturée(s).') % {'count': count},
            )
...
class AssetDepreciationAdmin(admin.ModelAdmin):
    ...
    def action_post(self, request, queryset):
        ...
        if 'apply' not in request.POST:
            ...
            return render(
                request,
                'admin/accounting/asset_depreciation/post_depreciation.html',
                context,
            )
        ...
        for depreciation in queryset.filter(state='draft'):
            try:
                ...
                try:
                    ...
                    errors.append(
                        f'{depreciation.asset_id.code}: {_("Aucune période fiscale ouverte pour cette date")}'
                    )
                    continue
        ...
        if count:
            self.message_user(
                request,
                _('%(count)d dotation(s) comptabilisée(s) avec succès.')
                % {'count': count},
            )
