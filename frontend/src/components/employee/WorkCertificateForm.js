// src/components/employee/WorkCertificateForm.js
...
const WorkCertificateForm = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      navigate('/my-space/certificates');
    } catch (error) {
    ...
    <div>
      ...
      <Card style={{ maxWidth: 600 }}>
        <Form form={form} layout="vertical" onFinish={handleSubmit} scrollToFirstError
          ...
          <Form.Item name="purpose" label="Objet de la demande" rules={[{ required: true }]}>
            <Select>
          ...
          <Space>
            <Button type="primary" htmlType="submit" loading={submitting}>Envoyer la demande</Button>
            <Button onClick={() => navigate('/my-space/certificates')}>Annuler</Button>
          </Space>
        </Form>
      </Card>
    </div>
