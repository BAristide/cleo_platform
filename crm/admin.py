...
class CompanyAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Métadonnées', {'fields': ('tags', 'created_by', 'assigned_to')}),
    )
...
class ContactAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Coordonnées', {'fields': ('email', 'phone', 'mobile')}),
        ('Profils sociaux', {'fields': ('linkedin', 'twitter')}),
        ('Source et suivi', {'fields': ('source', 'source_detail', 'notes')}),
        ('Métadonnées', {'fields': ('tags', 'created_by', 'assigned_to')}),
    )
...
class OpportunityAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Dates', {'fields': ('expected_close_date', 'closed_date')}),
        ('Statut', {'fields': ('is_closed', 'is_won', 'is_lost')}),
        ('Métadonnées', {'fields': ('tags', 'created_by', 'assigned_to')}),
    )
    ...
    def is_closed_display(self, obj):
        if obj.is_won:
            return format_html(
                '<span style="color: green; font-weight: bold;">Gagné</span>'
            )
        elif obj.is_lost:
            return format_html(
                '<span style="color: red; font-weight: bold;">Perdu</span>'
            )
        else:
...
class ActivityAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Relations', {'fields': ('company', 'opportunity', 'contacts')}),
        ('Rappel', {'fields': ('reminder', 'reminder_datetime')}),
        ('Métadonnées', {'fields': ('created_by', 'assigned_to')}),
    )
