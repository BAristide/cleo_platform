...
class Command(BaseCommand):
    help = "Crée des permissions personnalisées pour les actions clés dans chaque module"
    ...
    def handle(self, *args, **options):
        ...
        for app_label, perms in custom_permissions.items():
            ...
            try:
                ...
                if not model_list:
                    self.stdout.write(self.style.WARNING(f"Aucun modèle trouvé pour l'application {app_label}"))
                    continue
                ...
                for codename, name in perms:
                    ...
                    if created:
                        self.stdout.write(f"Permission créée: {permission.name} ({app_label}.{codename})")
                    else:
                        self.stdout.write(f"Permission existante: {permission.name} ({app_label}.{codename})")
            
            except LookupError:
                self.stdout.write(self.style.ERROR(f"Application '{app_label}' introuvable"))
