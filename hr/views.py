...
from .serializers import (
    ...
    MissionSerializer, AvailabilitySerializer,
    SkillSerializer, JobSkillRequirementSerializer,
    TrainingCourseSerializer, TrainingSkillSerializer,
    TrainingPlanSerializer, TrainingPlanItemSerializer
)
...
class EmployeeFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = Employee
        fields = ['department', 'manager', 'job_title', 'is_active', 'is_hr', 'is_finance']
...
class EmployeeViewSet(viewsets.ModelViewSet):
    ...
    ordering_fields = ['last_name', 'first_name', 'hire_date', 'department__name', 'job_title__name']
    ...
    def me(self, request):
        ...
        if user.is_authenticated:
            try:
                ...
                return Response({"error": "Aucun employé n'est associé à cet utilisateur."}, status=status.HTTP_404_NOT_FOUND)
        else:
...
class MissionViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['title', 'description', 'location', 'employee__first_name', 'employee__last_name']
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            elif serializer.validated_data.get('employee') and serializer.validated_data.get('employee').manager == employee:
                serializer.save(requested_by=employee, status='submitted')
            else:
    ...
    def submit(self, request, pk=None):
        ...
        if mission.status != 'draft':
            return Response({"error": "Cette mission n'est pas en mode brouillon."}, status=status.HTTP_400_BAD_REQUEST)
        
        mission.status = 'submitted'
        mission.save(update_fields=['status'])
        
        return Response({"success": True, "message": "Mission soumise pour approbation."})
    ...
    def approve_manager(self, request, pk=None):
        ...
        if mission.status != 'submitted':
            return Response({"error": "Cette mission n'est pas soumise pour approbation."}, status=status.HTTP_400_BAD_REQUEST)
        
        notes = request.data.get('notes', '')
        mission.approve_manager(notes)
        
        return Response({"success": True, "message": "Mission approuvée par le manager."})
    ...
    def approve_hr(self, request, pk=None):
        ...
        if mission.status != 'approved_manager':
            return Response({"error": "Cette mission n'est pas approuvée par le manager."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def approve_finance(self, request, pk=None):
        ...
        if mission.status != 'approved_hr':
            return Response({"error": "Cette mission n'est pas approuvée par les RH."}, status=status.HTTP_400_BAD_REQUEST)
        
        notes = request.data.get('notes', '')
        mission.approve_finance(notes)
        
        return Response({"success": True, "message": "Mission approuvée par la Finance."})
    ...
    def reject(self, request, pk=None):
        ...
        if mission.status in ['draft', 'completed', 'rejected', 'cancelled']:
            return Response({"error": "Impossible de rejeter cette mission dans son état actuel."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def submit_report(self, request, pk=None):
        ...
        if mission.status != 'approved_finance':
            return Response({"error": "Cette mission n'est pas approuvée par toutes les parties."}, status=status.HTTP_400_BAD_REQUEST)
        
        report = request.data.get('report', '')
        if not report:
            return Response({"error": "Le rapport ne peut pas être vide."}, status=status.HTTP_400_BAD_REQUEST)
        
        mission.report = report
        mission.report_submitted = True
        mission.report_date = timezone.now().date()
        mission.status = 'completed'
        mission.save(update_fields=['report', 'report_submitted', 'report_date', 'status'])
        
        return Response({"success": True, "message": "Rapport de mission soumis avec succès."})
    ...
    def generate_order_pdf(self, request, pk=None):
        ...
        try:
            ...
            return Response({"success": True, "message": "PDF d'ordre de mission généré avec succès.", "path": pdf_path})
    ...
    def download_order_pdf(self, request, pk=None):
        ...
        if os.path.exists(pdf_path):
            ...
            response['Content-Disposition'] = f'attachment; filename="Ordre_de_mission_{mission.id}.pdf"'
            return response
        else:
            return Response({"error": "Le fichier PDF n'existe pas."}, status=status.HTTP_404_NOT_FOUND)
...
class AvailabilityViewSet(viewsets.ModelViewSet):
    ...
    def approve_manager(self, request, pk=None):
        ...
        if availability.status != 'requested':
            return Response({"error": "Cette mise en disponibilité n'est pas en demande."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"success": True, "message": "Mise en disponibilité approuvée par le manager."})
    ...
    def approve_hr(self, request, pk=None):
        ...
        return Response({"success": True, "message": "Mise en disponibilité approuvée par les RH."})
...
class TrainingPlanViewSet(viewsets.ModelViewSet):
    ...
    def submit(self, request, pk=None):
        ...
        if plan.status != 'draft':
            return Response({"error": "Ce plan n'est pas en mode brouillon."}, status=status.HTTP_400_BAD_REQUEST)
        
        # Vérifier que le plan contient au moins un élément
        if not plan.training_items.exists():
            return Response({"error": "Le plan ne contient aucune formation."}, status=status.HTTP_400_BAD_REQUEST)
        
        plan.status = 'submitted'
        plan.save(update_fields=['status'])
        
        return Response({"success": True, "message": "Plan de formation soumis pour approbation."})
    ...
    def approve_manager(self, request, pk=None):
        ...
        if plan.status != 'submitted':
            return Response({"error": "Ce plan n'est pas soumis pour approbation."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"success": True, "message": "Plan de formation approuvé par le manager."})
    ...
    def approve_hr(self, request, pk=None):
        ...
        if plan.status != 'approved_manager':
            return Response({"error": "Ce plan n'est pas approuvé par le manager."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"success": True, "message": "Plan de formation approuvé par les RH."})
    ...
    def approve_finance(self, request, pk=None):
        ...
        if plan.status != 'approved_hr':
            return Response({"error": "Ce plan n'est pas approuvé par les RH."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"success": True, "message": "Plan de formation approuvé par la Finance."})
    ...
    def reject(self, request, pk=None):
        ...
        if plan.status in ['draft', 'completed', 'rejected']:
            return Response({"error": "Impossible de rejeter ce plan dans son état actuel."}, status=status.HTTP_400_BAD_REQUEST)
...
class TrainingPlanItemViewSet(viewsets.ModelViewSet):
    ...
    def schedule(self, request, pk=None):
        ...
        if item.status != 'planned':
            return Response({"error": "Cette formation n'est pas en statut planifiée."}, status=status.HTTP_400_BAD_REQUEST)
        
        scheduled_date = request.data.get('scheduled_date')
        if not scheduled_date:
            return Response({"error": "La date de programmation est requise."}, status=status.HTTP_400_BAD_REQUEST)
        
        item.scheduled_date = scheduled_date
        item.status = 'scheduled'
        item.save(update_fields=['scheduled_date', 'status'])
        
        return Response({"success": True, "message": "Formation programmée avec succès."})
    ...
    def start(self, request, pk=None):
        ...
        if item.status != 'scheduled':
            return Response({"error": "Cette formation n'est pas programmée."}, status=status.HTTP_400_BAD_REQUEST)
        
        item.status = 'in_progress'
        item.save(update_fields=['status'])
        
        return Response({"success": True, "message": "Formation marquée comme en cours."})
    ...
    def complete(self, request, pk=None):
        ...
        if item.status not in ['scheduled', 'in_progress']:
            return Response({"error": "Cette formation n'est pas programmée ou en cours."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not completion_date:
            return Response({"error": "La date de fin est requise."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"success": True, "message": "Formation marquée comme terminée."})
    ...
    def manager_evaluation(self, request, pk=None):
        ...
        if item.status != 'completed':
            return Response({"error": "Cette formation n'est pas terminée."}, status=status.HTTP_400_BAD_REQUEST)
        
        manager_rating = request.data.get('manager_rating')
        manager_comments = request.data.get('manager_comments', '')
        
        if not manager_rating:
            return Response({"error": "L'évaluation du manager est requise."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"success": True, "message": "Évaluation du manager ajoutée avec succès."})
...
def dashboard_view(request):
    ...
    employees_by_department = Employee.objects.filter(is_active=True).values('department__name').annotate(count=Count('id')).order_by('-count')
    employees_by_job_title = Employee.objects.filter(is_active=True).values('job_title__name', 'job_title__department__name').annotate(count=Count('id')).order_by('-count')
    
    # Missions en cours et à venir
    missions_by_status = Mission.objects.values('status').annotate(count=Count('id')).order_by('status')
    upcoming_missions = Mission.objects.filter(start_date__gte=timezone.now().date()).order_by('start_date')[:5]
    upcoming_missions_data = MissionSerializer(upcoming_missions, many=True).data
    
    # Formations
    training_plans_by_status = TrainingPlan.objects.filter(year=timezone.now().year).values('status').annotate(count=Count('id')).order_by('status')
    training_items_by_status = TrainingPlanItem.objects.filter(training_plan__year=timezone.now().year).values('status').annotate(count=Count('id')).order_by('status')
    
    # Compétences
    skills_count = Skill.objects.count()
    avg_skills_per_employee = EmployeeSkill.objects.values('employee').annotate(count=Count('id')).aggregate(avg=Avg('count'))['avg'] or 0
    ...
    for dept in Department.objects.all():
        ...
        if dept_data['employee_count'] > 0:
            ...
            for employee in employees:
                ...
                required_skills = JobSkillRequirement.objects.filter(job_title=employee.job_title)
