...
const InvoiceList = () => {
  ...
  const handleSearch = () => { setPagination({ ...pagination, current: 1 }); fetchInvoices(); };
  const resetFilters = () => {
    setSearchText(''); setStatusFilter('all'); setTypeFilter('all'); setDateRange(null);
    setPagination({ ...pagination, current: 1 }); fetchInvoices();
  };
  ...
  const handleTypeChange = (value) => { setTypeFilter(value); setPagination({ ...pagination, current: 1 }); };
        ...
        <Space size="small">
          ...
          {record.payment_status === 'unpaid' && record.type !== 'credit_note' && (
            <Button size="small" type="primary" icon={<DollarOutlined />}
              onClick={() => handleMarkAsPaid(record.id)} loading={actionLoading}
          ...
          {record.payment_status === 'unpaid' && record.type === 'standard' && (
            <Popconfirm title="Êtes-vous sûr de vouloir supprimer cette facture?"
              onConfirm={() => handleDeleteInvoice(record.id)} okText="Oui" cancelText="Non">
      ...
      <Card>
        ...
        <div style={{ marginBottom: 16 }}>
          <Row gutter={16}>
            ...
            <Col span={4}>
              <Select placeholder="Filtrer par type" style={{ width: '100%' }}
                value={typeFilter} onChange={handleTypeChange}>
