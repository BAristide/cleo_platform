...
class ProductAdmin(admin.ModelAdmin):
    list_display = ('reference', 'name', 'unit_price', 'currency', 'tax_rate', 'is_active')
...
class QuoteAdmin(admin.ModelAdmin):
    form = QuoteAdminForm
    list_display = ('number', 'company', 'contact', 'date', 'expiration_date',
                    'total_display', 'status', 'is_tax_exempt', 'pdf_status', 'email_status')
    list_filter = ('status', 'currency', 'date', 'expiration_date', 'is_tax_exempt')
    search_fields = ('number', 'company__name', 'contact__first_name', 'contact__last_name')
    date_hierarchy = 'date'
    readonly_fields = ('subtotal', 'tax_amount', 'total', 'pdf_file', 'email_sent', 'email_sent_date',
    ...
    fieldsets = (
        ...
        (_('Dates'), {
            'fields': ('date', 'expiration_date', 'validity_period')
        }),
        (_('Montants et taxes'), {
            'fields': ('currency', 'exchange_rate', 'bank_account', 'is_tax_exempt', 'tax_exemption_reason',
                      'discount_percentage', 'subtotal', 'tax_amount', 'total')
        }),
        (_('Conditions et notes'), {
            'fields': ('payment_terms', 'notes', 'terms')
        }),
        (_('Métadonnées'), {
            'classes': ('collapse',),
            'fields': ('created_at', 'updated_at', 'pdf_file', 'email_sent', 'email_sent_date')
        }),
    )
    ...
    actions = ['generate_pdf', 'send_by_email', 'convert_to_order', 'convert_to_invoice']
        ...
        for quote in queryset:
            if quote.contact and quote.contact.email:
                try:
                    ...
                    self.message_user(request, f"Erreur lors de l'envoi du devis {quote.number}: {str(e)}", level='ERROR')
            else:
                self.message_user(request, f"Le devis {quote.number} n'a pas d'email de contact défini.", level='WARNING')
    ...
    def convert_to_order(self, request, queryset):
        ...
        self.message_user(request, f"Tentative de conversion de {len(queryset)} devis en commandes...", level='INFO')

        for quote in queryset:
            # Afficher les informations sur le devis en cours de traitement
            self.message_user(request, f"Traitement du devis {quote.number} (statut: {quote.status})...", level='INFO')
            ...
            try:
                ...
                self.message_user(request, f"Appel de la méthode convert_to_order sur le devis {quote.number}...", level='INFO')
        ...
        if orders_created > 0:
            self.message_user(request, f"{orders_created} commandes ont été créées à partir des devis sélectionnés.")
        else:
            self.message_user(request, "Aucune commande n'a été créée. Vérifiez les messages ci-dessus pour plus de détails.", level='WARNING')
        ...
        for quote in queryset:
            if quote.status == 'accepted' and not quote.converted_to_invoice:
                try:
                    ...
                    self.message_user(request, f"Erreur lors de la conversion du devis {quote.number}: {str(e)}", level='ERROR')

        self.message_user(request, f"{invoices_created} factures ont été créées à partir des devis sélectionnés.")
...
class OrderAdmin(admin.ModelAdmin):
    form = OrderAdminForm
    list_display = ('number', 'company', 'contact', 'date', 'delivery_date',
                    'total_display', 'status', 'is_tax_exempt', 'pdf_status', 'email_status')
    ...
    fieldsets = (
        (_('Informations générales'), {
            'fields': ('number', 'company', 'contact', 'opportunity', 'quote', 'status')
        }),
        (_('Dates & Livraison'), {
    ...
    actions = ['generate_pdf', 'send_by_email', 'convert_to_invoice', 'create_deposit_invoice']

    def generate_pdf(self, request, queryset):
        for order in queryset:
            pdf_path = order.generate_pdf()
        self.message_user(request, f"{len(queryset)} commandes ont été générées en PDF.")
    generate_pdf.short_description = "Générer les PDFs"
        ...
        for order in queryset:
            if order.contact and order.contact.email:
                try:
                    ...
                    self.message_user(request, f"Erreur lors de l'envoi de la commande {order.number}: {str(e)}", level='ERROR')
            else:
                self.message_user(request, f"La commande {order.number} n'a pas d'email de contact défini.", level='WARNING')

        self.message_user(request, f"{sent_count} commandes ont été envoyées par email.")
    send_by_email.short_description = "Envoyer par email"
    ...
    def create_deposit_invoice(self, request, queryset):
        ...
        if queryset.count() != 1:
            self.message_user(request, "Veuillez sélectionner une seule commande pour créer une facture d'acompte.", level='ERROR')
            return
        ...
        try:
            ...
            deposit_invoice = Invoice.create_deposit_invoice(order, deposit_percentage=30)

            self.message_user(request,
                             f"Facture d'acompte {deposit_invoice.number} créée avec succès pour la commande {order.number}.",
                             level='SUCCESS')
        except Exception as e:
            self.message_user(request, f"Erreur lors de la création de la facture d'acompte: {str(e)}", level='ERROR')
    ...
    def convert_to_invoice(self, request, queryset):
        ...
        for order in queryset:
            ...
            try:
                ...
                deposit_total = deposit_invoices.aggregate(Sum('total'))['total__sum'] or Decimal('0')
                ...
                last_invoice = Invoice.objects.filter(type='standard').order_by('-id').first()
                ...
                for order_item in order_items:
                    InvoiceItem.objects.create(
                        ...
                        unit_price=order_item.unit_price * proportion,  # Ajuster le prix unitaire selon proportion restante
        ...
        if invoices_created == 0:
            self.message_user(request, "Aucune facture n'a été créée. Vérifiez les messages ci-dessus.", level='WARNING')
...
class InvoiceAdmin(admin.ModelAdmin):
    ...
    list_display = ('number', 'company', 'contact', 'date', 'due_date',
                    'total_display', 'payment_status', 'is_tax_exempt', 'pdf_status', 'email_status')
    list_filter = ('payment_status', 'currency', 'date', 'due_date', 'is_tax_exempt')
    search_fields = ('number', 'company__name', 'contact__first_name', 'contact__last_name')
    date_hierarchy = 'date'
    readonly_fields = ('subtotal', 'tax_amount', 'total', 'amount_paid', 'amount_due', 'pdf_file',
                       'email_sent', 'email_sent_date', 'created_at', 'updated_at')
    inlines = [InvoiceItemInline, PaymentInline]

    # Ajouter les nouveaux champs aux fieldsets
    fieldsets = (
        (_('Informations générales'), {
            'fields': ('number', 'type', 'company', 'contact', 'opportunity', 'quote', 'order', 'parent_invoice')
        }),
        (_('Détails du document'), {
            'fields': ('date', 'due_date', 'payment_terms', 'payment_status', 'amount_paid', 'amount_due')
        }),
    ...
    actions = ['generate_pdf', 'send_by_email', 'mark_as_paid', 'mark_as_partial', 'mark_as_overdue',
              'mark_as_unpaid', 'mark_as_cancelled', 'create_credit_note']
        ...
        for invoice in queryset:
            if invoice.contact and invoice.contact.email:
                try:
                    ...
                    self.message_user(request, f"Erreur lors de l'envoi de la facture {invoice.number}: {str(e)}", level='ERROR')
            else:
                self.message_user(request, f"La facture {invoice.number} n'a pas d'email de contact défini.", level='WARNING')
    ...
    def create_credit_note(self, request, queryset):
        ...
        if queryset.count() != 1:
            self.message_user(request, "Veuillez sélectionner une seule facture pour créer un avoir.", level='ERROR')
            return
        ...
        if getattr(invoice, 'type', 'standard') == 'credit_note':
            self.message_user(request, "Impossible de créer un avoir pour un autre avoir.", level='ERROR')
            return
        ...
        try:
            ...
            last_credit_note = Invoice.objects.filter(type='credit_note').order_by('-id').first()
            ...
            for item in invoice.get_items():
                InvoiceItem.objects.create(
                    ...
                    unit_price=-abs(item.unit_price),  # Montants négatifs pour les avoirs
    ...
    def mark_as_paid(self, request, queryset):
        ...
        self.message_user(request, f"Tentative de marquer {len(queryset)} factures comme payées...", level='INFO')
        updated_count = 0
        
        for invoice in queryset:
            if invoice.payment_status == 'paid':
                self.message_user(request, f"La facture {invoice.number} est déjà marquée comme payée.", level='WARNING')
                continue
            ...
            try:
                ...
                updated_count += 1
                self.message_user(request, f"Facture {invoice.number} marquée comme payée avec succès.", level='SUCCESS')
        ...
        if updated_count > 0:
            self.message_user(request, f"{updated_count} factures ont été marquées comme payées.")
        else:
            self.message_user(request, "Aucune facture n'a été marquée comme payée.", level='WARNING')
    ...
    def mark_as_partial(self, request, queryset):
        ...
        self.message_user(request, f"Tentative de marquer {len(queryset)} factures comme partiellement payées...", level='INFO')
        updated_count = 0

        for invoice in queryset:
            if invoice.payment_status == 'partial':
                self.message_user(request, f"La facture {invoice.number} est déjà marquée comme partiellement payée.", level='WARNING')
                continue
            ...
            try:
                ...
                with connection.cursor() as cursor:
                    ...
                    amount_paid = invoice.amount_paid if invoice.amount_paid > 0 else (invoice.total * Decimal('0.5'))
                ...
                updated_count += 1
                self.message_user(request, f"Facture {invoice.number} marquée comme partiellement payée.", level='SUCCESS')
        ...
        if updated_count > 0:
            self.message_user(request, f"{updated_count} factures ont été marquées comme partiellement payées.")
        else:
            self.message_user(request, "Aucune facture n'a été marquée comme partiellement payée.", level='WARNING')
    ...
    def update_invoice_status(self, request, queryset, status, status_display):
        ...
        for invoice in queryset:
            if invoice.payment_status == status:
                self.message_user(request, f"La facture {invoice.number} est déjà marquée comme {status_display}.", level='WARNING')
                continue
            ...
            try:
                ...
                updated_count += 1
                self.message_user(request, f"Facture {invoice.number} marquée comme {status_display}.", level='SUCCESS')
        ...
        if updated_count > 0:
            self.message_user(request, f"{updated_count} factures ont été marquées comme {status_display}.")
        else:
            self.message_user(request, f"Aucune facture n'a été marquée comme {status_display}.", level='WARNING')
