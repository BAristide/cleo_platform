...
class JournalSerializer(serializers.ModelSerializer):
    ...
    def get_default_debit_account_code(self, obj):
        return (
            obj.default_debit_account_id.code if obj.default_debit_account_id else None
        )

    def get_default_debit_account_name(self, obj):
        return (
            obj.default_debit_account_id.name if obj.default_debit_account_id else None
        )

    def get_default_credit_account_code(self, obj):
        return (
            obj.default_credit_account_id.code
            if obj.default_credit_account_id
            else None
        )

    def get_default_credit_account_name(self, obj):
        return (
            obj.default_credit_account_id.name
            if obj.default_credit_account_id
            else None
        )
...
class JournalEntryCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = JournalEntry
        fields = ['journal_id', 'date', 'ref', 'narration', 'lines']
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            raise serializers.ValidationError(
                'Aucune période fiscale ouverte pour cette date'
            )
...
class ReconciliationDetailSerializer(serializers.ModelSerializer):
    ...
    lines = JournalEntryLineSerializer(
        source='reconciled_lines', many=True, read_only=True
    )
    balance = serializers.SerializerMethodField()
...
class AssetSerializer(serializers.ModelSerializer):
    ...
    def get_net_book_value(self, obj):
        ...
        depreciation_sum = (
            obj.depreciation_lines.filter(state='posted').aggregate(sum=Sum('amount'))[
                'sum'
            ]
            or 0
        )
