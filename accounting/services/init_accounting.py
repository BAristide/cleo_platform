...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--journals', action='store_true', help=_('Initialiser les journaux')
        )
        parser.add_argument(
            '--fiscal-year',
            action='store_true',
        ...
        parser.add_argument(
            '--taxes', action='store_true', help=_('Initialiser les taxes')
        )
        parser.add_argument(
            '--analytic',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if options['accounts']:
            self.stdout.write(
                self.style.NOTICE(_('Initialisation des types de comptes...'))
            )
            init_service.create_account_types()
            self.stdout.write(
                self.style.NOTICE(_('Initialisation du plan comptable...'))
            )
        ...
        if options['fiscal_year']:
            self.stdout.write(
                self.style.NOTICE(_("Initialisation de l'exercice fiscal..."))
            )
        ...
        if options['analytic']:
            self.stdout.write(
                self.style.NOTICE(_('Initialisation des comptes analytiques...'))
            )
