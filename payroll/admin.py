...
class PayrollParameterAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Valeur', {'fields': ('value', 'effective_date', 'end_date')}),
...
class PayrollPeriodAdmin(admin.ModelAdmin):
    ...
    def close_periods(self, request, queryset):
        ...
        self.message_user(
            request, f'{queryset.count()} période(s) clôturée(s) avec succès.'
        )
...
class EmployeePayrollAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Employé', {'fields': ('employee',)}),
        (
            'Contrat et rémunération',
            {'fields': ('contract_type', 'base_salary', 'hourly_rate')},
        ),
        ('Identification fiscale et sociale', {'fields': ('cnss_number',)}),
        (
...
class PaySlipAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Identification', {'fields': ('payroll_run', 'employee', 'number')}),
        (
            'Données de présence',
            {
                'fields': (
        ...
        ('Documents', {'fields': ('pdf_file', 'notes')}),
    )
...
class PayrollRunAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Identification', {'fields': ('period', 'name', 'description', 'department')}),
        (
            'Statut et dates',
            {
                'fields': (
        ...
        ('Validation', {'fields': ('created_by', 'validated_by', 'notes')}),
    )
    ...
    def totals(self, obj):
        ...
        totals = payslips.aggregate(gross=Sum('gross_salary'), net=Sum('net_salary'))

        return format_html(
            'Brut: <b>{}</b> | Net: <b>{}</b>', totals['gross'] or 0, totals['net'] or 0
        )
    ...
    def generate_all_payslips(self, request, queryset):
        ...
        for payroll_run in queryset:
            if payroll_run.status != 'draft':
                self.message_user(
                    request,
                    f"Le lancement '{payroll_run.name}' n'est pas en brouillon, bulletins non générés.",
                    level='WARNING',
                )
                continue
            ...
            self.message_user(
                request,
                f"{created_count} bulletin(s) créé(s) pour le lancement '{payroll_run.name}'.",
            )
    ...
    def calculate_all_payslips(self, request, queryset):
        ...
        for payroll_run in queryset:
            if payroll_run.status not in ['in_progress', 'calculated']:
                self.message_user(
                    request,
                    f"Le lancement '{payroll_run.name}' n'est pas en cours ou calculé, bulletins non calculés.",
                    level='WARNING',
                )
                continue
            ...
            if not PaySlip.objects.filter(
                payroll_run=payroll_run, status='draft'
            ).exists():
            ...
            self.message_user(
                request,
                f"{calculated_count} bulletin(s) calculé(s) pour le lancement '{payroll_run.name}'.",
            )
    ...
    def validate_payroll(self, request, queryset):
        ...
        for payroll_run in queryset:
            if payroll_run.status != 'calculated':
                self.message_user(
                    request,
                    f"Le lancement '{payroll_run.name}' n'est pas calculé, non validé.",
                    level='WARNING',
                )
                continue
            ...
            if (
                ...
                self.message_user(
                    request,
                    f"Tous les bulletins du lancement '{payroll_run.name}' ne sont pas calculés, non validé.",
                    level='WARNING',
                )
                continue
    ...
    def mark_as_paid(self, request, queryset):
        ...
        for payroll_run in queryset:
            if payroll_run.status != 'validated':
                self.message_user(
                    request,
                    f"Le lancement '{payroll_run.name}' n'est pas validé, non marqué comme payé.",
                    level='WARNING',
                )
                continue
            ...
            PaySlip.objects.filter(payroll_run=payroll_run).update(
                status='paid', is_paid=True, payment_date=timezone.now().date()
            )
    ...
    def generate_summary_pdf(self, request, queryset):
        ...
        for payroll_run in queryset:
            if payroll_run.status not in ['calculated', 'validated', 'paid']:
                self.message_user(
                    request,
                    f"Le lancement '{payroll_run.name}' n'est pas calculé, validé ou payé, PDF non généré.",
                    level='WARNING',
                )
                continue

            try:
                PayrollPDFGenerator.generate_payroll_run_summary(payroll_run)
                generated += 1
            except Exception as e:
        ...
        self.message_user(
            request, f'{generated} PDF récapitulatif(s) généré(s) avec succès.'
        )
...
class AdvanceSalaryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Employé et période', {'fields': ('employee', 'period')}),
        ('Montant et paiement', {'fields': ('amount', 'payment_date', 'is_paid')}),
        ('Bulletin', {'fields': ('payslip', 'notes')}),
    )
