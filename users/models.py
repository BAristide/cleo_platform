...
class ModulePermission(models.Model):
    ...
    role = models.ForeignKey(
        UserRole, on_delete=models.CASCADE, related_name='module_permissions'
    )
    module = models.CharField(_('Module'), max_length=20, choices=MODULE_CHOICES)
    access_level = models.CharField(
        _("Niveau d'accès"), max_length=20, choices=ACCESS_LEVELS, default='no_access'
    )
        ...
        for ct in content_types:
            ...
            if self.access_level in ['delete', 'admin']:
                delete_perms = Permission.objects.filter(
                    content_type=ct, codename__startswith='delete_'
                )
...
class ActivityLog(models.Model):
    ...
    module = models.CharField(
        _('Module'), max_length=20, choices=ModulePermission.MODULE_CHOICES
    )
