...
from .serializers import (
    ...
    ActivityLogSerializer, EmployeeUserLinkSerializer
)
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['username', 'email', 'first_name', 'last_name', 'profile__employee__first_name', 'profile__employee__last_name']
    ...
    def add_to_role(self, request, pk=None):
        ...
        if not role_id:
            return Response({"error": "L'ID du rôle est requis"}, status=status.HTTP_400_BAD_REQUEST)
        
        try:
            role = UserRole.objects.get(pk=role_id)
            group = role.group
            user.groups.add(group)
            return Response({"success": f"L'utilisateur a été ajouté au rôle {role.name}"})
    ...
    def remove_from_role(self, request, pk=None):
        ...
        try:
            ...
            return Response({"success": f"L'utilisateur a été retiré du rôle {role.name}"})
...
class UserRoleViewSet(viewsets.ModelViewSet):
    ...
    def set_module_permissions(self, request, pk=None):
        ...
        if not permissions_data:
            return Response({"error": "Les données de permissions sont requises"}, status=status.HTTP_400_BAD_REQUEST)
        
        # Supprimer les permissions existantes pour les modules concernés
        modules_to_update = [perm['module'] for perm in permissions_data if 'module' in perm]
        ModulePermission.objects.filter(role=role, module__in=modules_to_update).delete()
...
class ActivityLogViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if backend.has_module_permission(self.request.user, 'core', 'admin'):
            return queryset
...
def log_activity(request):
    ...
    if not request.user.is_superuser:
        backend = ModulePermissionBackend()
        if not backend.has_module_permission(request.user, 'core', 'create'):
