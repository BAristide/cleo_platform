...
class PayrollPDFGenerator:
    ...
    def generate_payslip_pdf(payslip):
        ...
        output_dir = os.path.join(settings.MEDIA_ROOT, 'payslips', str(payslip.payroll_run.period.id))
        ...
        relative_path = os.path.join('payslips', str(payslip.payroll_run.period.id), filename)
    ...
    def generate_payroll_run_summary(payroll_run):
        ...
        context = {
            ...
            'payslips': payroll_run.payslips.all().order_by('employee__last_name', 'employee__first_name'),
            'summary': {
                ...
                'total_cnss_employee': sum(p.cnss_employee for p in payroll_run.payslips.all()),
                'total_cnss_employer': sum(p.cnss_employer for p in payroll_run.payslips.all()),
                'total_amo_employee': sum(p.amo_employee for p in payroll_run.payslips.all()),
                'total_amo_employer': sum(p.amo_employer for p in payroll_run.payslips.all()),
                'total_income_tax': sum(p.income_tax for p in payroll_run.payslips.all()),
            },
            'company': {
                'name': 'ECINTELLIGENCE',
                'address': 'La Marina Casablanca | Tour Oceanes 3 Bureau 03 Rez-De-Jardin',
        ...
        # Créer le répertoire de sortie si nécessaire
        output_dir = os.path.join(settings.MEDIA_ROOT, 'payroll_reports', str(payroll_run.period.id))
        os.makedirs(output_dir, exist_ok=True)
        
        # Nom du fichier PDF
        filename = f"récapitulatif_paie_{payroll_run.name}_{payroll_run.period.name}.pdf"
        output_path = os.path.join(output_dir, filename)
