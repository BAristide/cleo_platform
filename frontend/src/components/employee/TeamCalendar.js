// src/components/employee/TeamCalendar.js
...
const TeamCalendar = () => {
  ...
  const fetchLeaves = useCallback(async (date, departmentId) => {
    ...
    try {
      ...
      setError('Impossible de charger le calendrier des congés.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => { fetchLeaves(currentDate, selectedDepartment); }, [currentDate, selectedDepartment, fetchLeaves]);
        ...
        {dayLeaves.slice(0, 3).map((l) => (
          <li key={l.id}>
            <Tooltip title={`${l.leave_type_name} — ${l.nb_days}j`}>
              <Badge color={l.leave_type_color || '#1890ff'}
                text={<span style={{ fontSize: 11 }}>{l.employee_name}</span>} />
            </Tooltip>
          </li>
        ))}
        {dayLeaves.length > 3 && (
          <li><Tag color="default" style={{ fontSize: 10 }}>+{dayLeaves.length - 3} autre(s)</Tag></li>
        )}
      </ul>
    );
  };
  ...
  return (
    <div style={{ padding: '24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 16, gap: 16 }}>
        ...
        <h2 style={{ margin: 0 }}>Calendrier des congés équipe</h2>
      </div>
      {error && <Alert type="error" message={error} style={{ marginBottom: 16 }} closable />}
      <Card extra={
        <Select placeholder="Tous les départements" allowClear style={{ width: 220 }}
          onChange={(v) => setSelectedDepartment(v || null)} value={selectedDepartment}>
          {departments.map((d) => (<Option key={d.id} value={d.id}>{d.name}</Option>))}
        </Select>
      }>
        <Spin spinning={loading}>
          <Calendar dateCellRender={dateCellRender} onPanelChange={setCurrentDate}
            value={currentDate} onChange={setCurrentDate} />
        </Spin>
      </Card>
      <div style={{ marginTop: 12, color: '#888', fontSize: 12 }}>
        Affiche les congés avec statut <strong>Approuvé RH</strong> uniquement.
        {leaves.length > 0 && ` — ${leaves.length} congé(s) ce mois.`}
      </div>
    </div>
  );
};
