...
class JobOpeningAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('URL de candidature', {'fields': ('application_url',)}),
        (
            'Métadonnées',
    ...
    def applications_count(self, obj):
        ...
        return format_html(
            '<a href="{}?job_opening__id__exact={}">{} candidature(s)</a>',
...
class CandidateAdmin(admin.ModelAdmin):
    ...
    def applications_count(self, obj):
        count = obj.applications.count()
        return format_html(
            '<a href="{}?candidate__id__exact={}">{} candidature(s)</a>',
            reverse('admin:recruitment_application_changelist'),
            obj.id,
            count,
        )
...
class CandidateEvaluationInline(admin.TabularInline):
    ...
    def evaluation_form_link(self, obj):
        if obj.evaluation_form:
            return format_html(
                '<a href="{}" target="_blank">Voir la fiche</a>',
                obj.evaluation_form.url,
            )
        return '-'
...
class ApplicationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Entretien', {'fields': ('interview_date', 'interview_location', 'notes')}),
        (
            'Métadonnées',
            {'fields': ('created_at', 'updated_at'), 'classes': ('collapse',)},
        ),
    ...
    def resume_link(self, obj):
        if obj.resume:
            return format_html(
                '<a href="{}" target="_blank">Télécharger</a>', obj.resume.url
            )
        return '-'
    ...
    def cover_letter_link(self, obj):
        if obj.cover_letter:
            return format_html(
                '<a href="{}" target="_blank">Télécharger</a>', obj.cover_letter.url
            )
        return '-'
    ...
    def reject_applications(self, request, queryset):
        ...
        rejected_screening = queryset.filter(status='received').update(
            status='rejected_screening'
        )
        rejected_analysis = queryset.filter(status='preselected').update(
            status='rejected_analysis'
        )
        rejected_interview = queryset.filter(status='interviewed').update(
    ...
    def schedule_interview(self, request, queryset):
        updated = queryset.filter(status='preselected').update(
            status='selected_for_interview'
        )
        self.message_user(
            request, f'{updated} candidature(s) sélectionnée(s) pour entretien.'
        )
...
class CandidateEvaluationAdmin(admin.ModelAdmin):
    ...
    search_fields = (
        ...
        'interviewer__employee__first_name',
        'interviewer__employee__last_name',
    )
    ...
    fieldsets = (
        ...
        ('Notes', {'fields': ('strengths', 'weaknesses', 'total_score')}),
        ('Document', {'fields': ('evaluation_form',)}),
        (
            'Métadonnées',
            {'fields': ('created_at', 'updated_at'), 'classes': ('collapse',)},
        ),
