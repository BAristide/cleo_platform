// src/components/employee/LeaveBalanceDashboard.js
    ...
    <div>
      <div style={{ marginBottom: 16, textAlign: 'right' }}>
        <Button
          type="primary"
          icon={<PlusOutlined />}
          onClick={() => navigate('/my-space/leaves/new')}
        >
          Nouvelle demande
        </Button>
      </div>
            ...
            return (
              <Col xs={24} sm={12} lg={8} key={alloc.id}>
                    ...
                    <span>
                      <span
                        style={{
                          ...
                          borderRadius: '50%', backgroundColor: alloc.leave_type_color || '#1890ff',
                          marginRight: 8,
                        }}
                      />
                ...
                >
                  <Row gutter={8}>
                    <Col span={8}>
                      <Statistic title="Solde" value={remaining} suffix="j" precision={1}
                        valueStyle={{ fontSize: 20, color: remaining <= 0 ? '#ff4d4f' : '#52c41a' }} />
                    </Col>
                    <Col span={8}>
                      <Statistic title="Utilisés" value={alloc.used_days} suffix="j" precision={1}
                        valueStyle={{ fontSize: 20, color: '#fa8c16' }} />
                    </Col>
                    <Col span={8}>
                      <Statistic title="En attente" value={alloc.pending_days} suffix="j" precision={1}
