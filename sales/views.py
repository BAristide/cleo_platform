...
from .serializers import (
    BankAccountSerializer, ProductSerializer,
...
class InvoiceFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = Invoice
        fields = ['type', 'payment_status', 'currency', 'company', 'contact', 'quote', 'order', 'parent_invoice']
...
class OrderFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = Order
        fields = ['status', 'currency', 'company', 'contact', 'quote', 'converted_to_invoice', 'has_deposit_invoice', 'has_final_invoice']
...
class QuoteFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = Quote
        fields = ['status', 'currency', 'company', 'contact', 'converted_to_order', 'converted_to_invoice']
...
class BankAccountViewSet(viewsets.ModelViewSet):
    ...
    def default(self, request):
        ...
        try:
            account = BankAccount.objects.filter(currency_id=currency_id, is_default=True).first()
            if account:
                serializer = self.get_serializer(account)
                return Response(serializer.data)
            else:
                return Response({"error": "No default account found for this currency"}, status=status.HTTP_404_NOT_FOUND)
...
class QuoteViewSet(viewsets.ModelViewSet):
    ...
    def generate_pdf(self, request, pk=None):
        ...
        try:
            ...
            if os.path.exists(pdf_path):
                return Response({"success": True, "message": "PDF generated successfully", "path": pdf_path})
            else:
    ...
    def accept(self, request, pk=None):
        ...
        if quote.status in ['expired', 'rejected', 'cancelled']:
            return Response({"error": "Cannot accept a quote that is expired, rejected, or cancelled"},
    ...
    def reject(self, request, pk=None):
        ...
        if quote.status in ['accepted', 'expired', 'cancelled']:
            return Response({"error": "Cannot reject a quote that is accepted, expired, or cancelled"},
                           status=status.HTTP_400_BAD_REQUEST)
    ...
    def cancel(self, request, pk=None):
        ...
        if quote.status in ['expired', 'rejected']:
            return Response({"error": "Cannot cancel a quote that is already expired or rejected"},
                           status=status.HTTP_400_BAD_REQUEST)

        if quote.converted_to_order or quote.converted_to_invoice:
            return Response({"error": "Cannot cancel a quote that has been converted to order or invoice"},
                           status=status.HTTP_400_BAD_REQUEST)
    ...
    def convert_to_order(self, request, pk=None):
        ...
        if quote.status != 'accepted':
            return Response({"error": "Only accepted quotes can be converted to orders"},
                           status=status.HTTP_400_BAD_REQUEST)

        if quote.converted_to_order:
            return Response({"error": "This quote has already been converted to an order"},
                           status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({"success": True, "message": "Quote converted to order successfully", "order": serializer.data})
    ...
    def convert_to_invoice(self, request, pk=None):
        ...
        if quote.status != 'accepted':
            return Response({"error": "Only accepted quotes can be converted to invoices"},
                           status=status.HTTP_400_BAD_REQUEST)

        if quote.converted_to_invoice:
            return Response({"error": "This quote has already been converted to an invoice"},
                           status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({"success": True, "message": "Quote converted to invoice successfully", "invoice": serializer.data})
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def download_pdf(self, request, pk=None):
        ...
        if os.path.exists(pdf_path):
            response = FileResponse(open(pdf_path, 'rb'), content_type='application/pdf')
            response['Content-Disposition'] = f'attachment; filename="Bon_de_commande_{order.number}.pdf"'
            return response
        else:
            return Response({"error": "PDF not found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def cancel(self, request, pk=None):
        ...
        if order.status in ['delivered', 'cancelled']:
            return Response({"error": "Cannot cancel an order that is already delivered or cancelled"},
                           status=status.HTTP_400_BAD_REQUEST)

        if order.converted_to_invoice:
            return Response({"error": "Cannot cancel an order that has been converted to invoice"},
                           status=status.HTTP_400_BAD_REQUEST)
    ...
    def convert_to_invoice(self, request, pk=None):
        ...
        if order.status != 'confirmed':
            return Response({"error": "Only confirmed orders can be converted to invoices"},
                           status=status.HTTP_400_BAD_REQUEST)

        if order.converted_to_invoice:
            return Response({"error": "This order has already been converted to an invoice"},
                           status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if remaining_amount <= 0:
                return Response({"error": "The total amount of the order has already been invoiced via deposit invoices"},
                              status=status.HTTP_400_BAD_REQUEST)
            ...
            # Retourner la facture créée
            serializer = InvoiceSerializer(invoice)
            return Response({"success": True, "message": "Order converted to invoice successfully", "invoice": serializer.data})
    ...
    def create_deposit_invoice(self, request, pk=None):
        ...
        if not order.can_create_deposit_invoice():
            return Response({"error": "This order cannot have a deposit invoice (invalid status or amount already invoiced)"},
                           status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({"success": True, "message": f"Deposit invoice created successfully with {deposit_percentage}% of the order value", "invoice": serializer.data})
        except Exception as e:
            return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class InvoiceViewSet(viewsets.ModelViewSet):
    ...
    def mark_as_paid(self, request, pk=None):
        ...
        if amount_to_pay > 0:
            ...
            try:
                ...
                return Response({"success": True, "message": "Invoice marked as paid", "payment": serializer.data})

            except Exception as e:
                return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        else:
            return Response({"error": "Invoice amount is zero or negative"},
                          status=status.HTTP_400_BAD_REQUEST)
    ...
    def create_credit_note(self, request, pk=None):
        ...
        if invoice.type == 'credit_note':
            return Response({"error": "Cannot create a credit note for another credit note"},
                           status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if 'amount' in request.data:
                ...
                if credit_amount <= 0 or credit_amount > invoice.total:
                    return Response({"error": "Credit amount must be positive and not exceed the invoice total"},
                                   status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({"success": True, "message": "Credit note created successfully", "credit_note": serializer.data})
        except Exception as e:
            return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class InvoiceItemViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        instance.invoice.save(update_fields=['subtotal', 'tax_amount', 'total', 'amount_due'])
