// src/components/hr/EmployeePortal.js
...
import {
  UserOutlined, CarOutlined, CalendarOutlined,
...
const MISSION_STATUS = {
  ...
  approved_manager: { label: 'Approuvee N+1', color: 'processing' },
  approved_hr: { label: 'Approuvee RH', color: 'processing' },
  approved_finance: { label: 'Approuvee Finance', color: 'success' },
...
const EmployeePortal = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (err.response?.status === 404) {
          setError("Aucun dossier employe n'est associe a votre compte. Contactez les RH.");
        } else {
          setError("Impossible de charger votre portail employe.");
        }
      } finally {
        setLoading(false);
      }
  ...
  const missionColumns = [
    ...
    { title: 'Debut', dataIndex: 'start_date', key: 'start_date' },
  ...
  const availabilityColumns = [
    ...
    { title: 'Debut', dataIndex: 'start_date', key: 'start_date' },
    { title: 'Fin', dataIndex: 'end_date', key: 'end_date' },
    { title: 'Duree (jours)', dataIndex: 'duration_days', key: 'duration_days' },
    {
      title: 'Statut',
      dataIndex: 'status',
      key: 'status',
  ...
  const trainingColumns = [
    { title: 'Annee', dataIndex: 'year', key: 'year' },
    ...
    },
    {
      ...
      key: 'detail',
      render: (_, record) => (
        <Link to={`/hr/training-plans/${record.id}`}>Voir</Link>
      ),
    },
  ];

  const payslipColumns = [
    { title: 'Periode', dataIndex: 'period_display', key: 'period_display' },
    {
      title: 'Salaire brut',
  ...
  if (error) {
    return (
      <div style={{ padding: 24 }}>
        <Alert message="Portail employe" description={error} type="warning" showIcon />
      </div>
    );
  }
  ...
  return (
    <div className="hr-employee-portal">
      <Title level={2}>Mon espace employe</Title>
        ...
        <Card title="Informations contractuelles" style={{ marginBottom: 24 }}>
          <Descriptions column={{ xs: 1, sm: 2, md: 3 }} size="small">
            ...
            {employee.contract_start_date && (
              <Descriptions.Item label="Debut contrat">
                {employee.contract_start_date}
              </Descriptions.Item>
            )}
      ...
      <Tabs defaultActiveKey="missions">
        ...
        >
          <div style={{ marginBottom: 12, textAlign: 'right' }}>
            <Button
              type="primary"
              icon={<PlusOutlined />}
              onClick={() => navigate('/hr/missions/new')}
            >
              Nouvelle mission
            </Button>
          </div>
        ...
        <TabPane
          tab={<span><CalendarOutlined /> Disponibilites ({availabilities.length})</span>}
          key="availabilities"
        >
          <div style={{ marginBottom: 12, textAlign: 'right' }}>
            <Button
              type="primary"
              icon={<PlusOutlined />}
              onClick={() => navigate('/hr/availabilities/new')}
            >
              Nouvelle demande
            </Button>
          </div>
          {availabilities.length === 0 ? (
            <Empty description="Aucune demande de disponibilite" />
          ) : (
            <Table
        ...
        <TabPane
          tab={<span><BookOutlined /> Formations ({trainingPlans.length})</span>}
          key="training"
        >
        ...
        <TabPane
          tab={<span><FileTextOutlined /> Bulletins de paie ({payslips.length})</span>}
          key="payslips"
        >
          {payslips.length === 0 ? (
