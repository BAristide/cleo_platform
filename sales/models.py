...
class SalesDocument(models.Model):
    ...
    # Champs financiers
    subtotal = models.DecimalField(_("Sous-total HT"), max_digits=15, decimal_places=2, default=0)
    tax_amount = models.DecimalField(_("Montant TVA"), max_digits=15, decimal_places=2, default=0)
    total = models.DecimalField(_("Total TTC"), max_digits=15, decimal_places=2, default=0)

    # Nouveau champ pour la remise
    discount_percentage = models.DecimalField(_("Remise (%)"), max_digits=5, decimal_places=2, default=0.00)

    # Devise et taux de change
    currency = models.ForeignKey(Currency, on_delete=models.CASCADE, verbose_name=_("Devise"))
    exchange_rate = models.DecimalField(_("Taux de change"), max_digits=10, decimal_places=4, default=1.0000)

    # Conditions de paiement
    payment_terms = models.CharField(_("Conditions de paiement"), max_length=20, choices=[
    ...
    tax_exemption_reason = models.CharField(_("Raison d'exonération"), max_length=255, blank=True, null=True)
    ...
    email_sent_date = models.DateTimeField(_("Date d'envoi de l'email"), blank=True, null=True)
        ...
        if self.pk:
            ...
            if items.exists():
                ...
                if self.discount_percentage > 0:
                    discount_amount = (self.subtotal * self.discount_percentage) / Decimal('100')
                ...
                if not getattr(self, 'is_tax_exempt', False):
                    for item in items:
                        ...
                        item_proportion = (item.quantity * item.unit_price) / self.subtotal if self.subtotal > 0 else 0

                        # Appliquer la remise proportionnellement à cet item
                        item_after_discount = (item.quantity * item.unit_price) - (discount_amount * item_proportion)

                        # Calculer la TVA sur le montant après remise
                        item_tax = item_after_discount * (item.tax_rate / Decimal('100'))
        ...
        if hasattr(self, 'currency') and self.currency:
            ...
            if default_currency and self.currency != default_currency:
                ...
                if not self.tax_exemption_reason:
                    self.tax_exemption_reason = "Exonération de TVA pour facturation en devise étrangère"
    ...
    def get_template_name(self):
        ...
        """
        raise NotImplementedError("Les sous-classes doivent implémenter get_template_name()")
    ...
    def get_pdf_filename(self):
        ...
        À surcharger dans les sous-classes.
        """
        raise NotImplementedError("Les sous-classes doivent implémenter get_pdf_filename()")
    ...
    def get_email_template(self):
        ...
        À surcharger dans les sous-classes.
        """
        raise NotImplementedError("Les sous-classes doivent implémenter get_email_template()")
    ...
    def get_email_subject(self):
        ...
        À surcharger dans les sous-classes.
        """
        raise NotImplementedError("Les sous-classes doivent implémenter get_email_subject()")
    ...
    def send_by_email(self, recipient_email=None):
        ...
        if not self.pdf_file or not os.path.exists(os.path.join(settings.MEDIA_ROOT, self.pdf_file)):
...
class Product(models.Model):
    ...
    currency = models.ForeignKey(Currency, on_delete=models.CASCADE, verbose_name=_("Devise"))
    tax_rate = models.DecimalField(_("Taux de TVA (%)"), max_digits=5, decimal_places=2, default=20.00)
...
class Quote(SalesDocument):
    ...
    """
    company = models.ForeignKey('crm.Company', on_delete=models.CASCADE, verbose_name=_("Entreprise"))
    contact = models.ForeignKey('crm.Contact', on_delete=models.CASCADE, verbose_name=_("Contact"))
    opportunity = models.ForeignKey('crm.Opportunity', on_delete=models.CASCADE,
                                    verbose_name=_("Opportunité"), null=True, blank=True)
    ...
    validity_period = models.PositiveIntegerField(_("Période de validité (jours)"), default=20)
    ...
    bank_account = models.ForeignKey(BankAccount, on_delete=models.CASCADE,
                                     verbose_name=_("Compte bancaire"), null=True, blank=True)
    ...
    def send_by_email(self, recipient_email=None):
        ...
        email_sent = EmailService.send_quote_email(
            self,
            recipient_email,
            f"Devis {self.number} - ECINTELLIGENCE"
        )
    ...
    def convert_to_order(self):
        ...
        if self.converted_to_order:
            error_msg = f"Ce devis a déjà été converti en commande"
            logger.error(error_msg)
            raise ValueError(error_msg)
        ...
        logger.info(f"Lignes copiées, marquage du devis comme converti")

        # Marquer le devis comme converti
        self.converted_to_order = True
        self.save(update_fields=['converted_to_order'])

        logger.info(f"Conversion terminée avec succès: devis {self.number} -> commande {order_number}")
    ...
    def convert_to_invoice(self):
        ...
        """
        if self.status != 'accepted':
            raise ValueError("Seuls les devis acceptés peuvent être convertis en factures")
...
class QuoteItem(models.Model):
    ...
    description = models.TextField(_("Description"), blank=True)
    quantity = models.DecimalField(_("Quantité"), max_digits=10, decimal_places=2, default=1)
    unit_price = models.DecimalField(_("Prix unitaire"), max_digits=15, decimal_places=2)
    tax_rate = models.DecimalField(_("Taux de TVA (%)"), max_digits=5, decimal_places=2, default=20.00)
...
class Order(SalesDocument):
    ...
    quote = models.ForeignKey(Quote, on_delete=models.SET_NULL,
                              verbose_name=_("Devis d'origine"), null=True, blank=True)
    ...
    delivery_address = models.TextField(_("Adresse de livraison"), blank=True, null=True)
    ...
    has_deposit_invoice = models.BooleanField(_("A une facture d'acompte"), default=False)
    ...
    def can_create_deposit_invoice(self):
        ...
        return self.status in ['confirmed', 'in_progress'] and not self.has_deposit_invoice

    def can_create_final_invoice(self):
        """Vérifie si une facture finale peut être créée."""
        return self.status in ['confirmed', 'in_progress', 'delivered'] and not self.has_final_invoice and self.get_remaining_amount() > 0
    ...
    def save(self, *args, **kwargs):
        ...
        if 'status' in kwargs.get('update_fields', []) or not kwargs.get('update_fields'):
        ...
        if hasattr(self, '_status') and self._status != self.status:
            print(f"ALERTE: Le statut n'a pas été correctement enregistré. Tentative supplémentaire.")
    ...
    def send_by_email(self, recipient_email=None):
        ...
        email_sent = EmailService.send_order_email(
            self,
            recipient_email,
            f"Commande {self.number} - ECINTELLIGENCE"
        )
...
class OrderItem(models.Model):
    ...
    """
    order = models.ForeignKey(Order, on_delete=models.CASCADE, verbose_name=_("Commande"))
    product = models.ForeignKey(Product, on_delete=models.CASCADE, verbose_name=_("Produit"))
    description = models.TextField(_("Description"), blank=True)
    quantity = models.DecimalField(_("Quantité"), max_digits=10, decimal_places=2, default=1)
    unit_price = models.DecimalField(_("Prix unitaire"), max_digits=15, decimal_places=2)
...
class Invoice(SalesDocument):
    ...
    order = models.ForeignKey(Order, on_delete=models.SET_NULL,
                              verbose_name=_("Commande d'origine"), null=True, blank=True)

    # Suivi des paiements
    payment_status = models.CharField(_("Statut de paiement"), max_length=20, choices=[
        ('unpaid', _('Non payée')),
    ...
    amount_paid = models.DecimalField(_("Montant payé"), max_digits=15, decimal_places=2, default=0)
    amount_due = models.DecimalField(_("Montant dû"), max_digits=15, decimal_places=2, default=0)
    ...
    type = models.CharField(_("Type de facture"), max_length=20, choices=INVOICE_TYPE_CHOICES, default='standard')
    
    # Références aux factures liées
    parent_invoice = models.ForeignKey('self', on_delete=models.SET_NULL, null=True, blank=True,
                                      related_name='child_invoices', verbose_name=_("Facture d'origine"))
    
    # Pour les factures d'acompte, stocker le pourcentage d'acompte
    deposit_percentage = models.DecimalField(_("Pourcentage d'acompte"), max_digits=5, decimal_places=2, 
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.number:
            ...
            if hasattr(self, 'type'):
                last_invoice = Invoice.objects.filter(type=self.type).order_by('-id').first()
            else:
        ...
        if 'payment_status' in kwargs.get('update_fields', []) or not kwargs.get('update_fields'):
            # Mettre à jour explicitement l'attribut sur l'instance
        ...
        if self.payment_status != 'cancelled' and ('payment_status' not in kwargs.get('update_fields', [])):
        ...
        # Vérifier si le statut a été correctement enregistré (contournement du problème)
        if hasattr(self, '_payment_status') and self._payment_status != self.payment_status:
            print(f"ALERTE: Le statut de paiement n'a pas été correctement enregistré. Tentative supplémentaire.")
            print(f"  Statut attendu: {self._payment_status}, Statut actuel: {self.payment_status}")
            # Tentative supplémentaire avec une requête directe
            from django.db import connection
            with connection.cursor() as cursor:
                cursor.execute(
        ...
        if hasattr(self, 'type') and self.type == 'credit_note' and hasattr(self, 'parent_invoice') and self.parent_invoice:
            try:
                # Mettre à jour le montant payé de la facture d'origine
                parent = self.parent_invoice
                parent.amount_paid = max(0, parent.amount_paid + self.total)  # self.total est négatif pour un avoir
                parent.amount_due = parent.total - parent.amount_paid
                parent._update_payment_status()
                parent.save(update_fields=['amount_paid', 'amount_due', 'payment_status'])
            except Exception as e:
                print(f"Erreur lors de la mise à jour de la facture d'origine: {str(e)}")
    ...
    def send_by_email(self, recipient_email=None):
        ...
        email_sent = EmailService.send_invoice_email(
            self,
            recipient_email,
            f"Facture {self.number} - ECINTELLIGENCE"
        )
    ...
    def create_deposit_invoice(cls, order, deposit_percentage=30):
        ...
        deposit_invoice = cls.objects.create(
            ...
            tax_amount=(order.tax_amount * Decimal(deposit_percentage)) / Decimal('100'),
        ...
        for order_item in order_items:
            ...
            item_subtotal = (order_item.quantity * order_item.unit_price * Decimal(deposit_percentage)) / Decimal('100')
            item_tax = item_subtotal * (order_item.tax_rate / Decimal('100')) if not order.is_tax_exempt else Decimal('0')
            ...
            InvoiceItem.objects.create(
                ...
                unit_price=(order_item.unit_price * Decimal(deposit_percentage)) / Decimal('100'),  # Ajuster le prix unitaire
...
class InvoiceItem(models.Model):
    ...
    """
    invoice = models.ForeignKey(Invoice, on_delete=models.CASCADE, verbose_name=_("Facture"))
    product = models.ForeignKey(Product, on_delete=models.CASCADE, verbose_name=_("Produit"))
    description = models.TextField(_("Description"), blank=True)
    quantity = models.DecimalField(_("Quantité"), max_digits=10, decimal_places=2, default=1)
    unit_price = models.DecimalField(_("Prix unitaire"), max_digits=15, decimal_places=2)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.invoice:
            ...
            self.invoice.save(update_fields=['subtotal', 'tax_amount', 'total', 'amount_due'])
...
class Payment(models.Model):
    ...
    method = models.CharField(_("Méthode de paiement"), max_length=20, choices=[
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            ...
            self.invoice.save(update_fields=['amount_paid', 'amount_due', 'payment_status'])
