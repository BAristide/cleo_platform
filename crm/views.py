...
def dashboard_view(request):
    ...
    pipeline_value = Opportunity.objects.aggregate(total=Sum('amount'))['total'] or 0
...
class CompanyViewSet(viewsets.ModelViewSet):
    ...
    def stats(self, request):
        ...
        return Response(
            {'total_companies': total_companies, 'by_industry': companies_by_industry}
        )
...
class ContactViewSet(viewsets.ModelViewSet):
    ...
    def stats(self, request):
        ...
        return Response(
            {'total_contacts': total_contacts, 'by_source': contacts_by_source}
        )
...
class OpportunityViewSet(viewsets.ModelViewSet):
    ...
    def stats(self, request):
        ...
        won_value = (
            Opportunity.objects.filter(stage__is_won=True).aggregate(
                total=Sum('amount')
            )['total']
            or 0
        )
        ...
        by_stage = (
            ...
            .annotate(count=Count('id'), total_value=Sum('amount'))
...
class ActivityViewSet(viewsets.ModelViewSet):
    ...
    def upcoming(self, request):
        ...
        activities = Activity.objects.filter(
            status='planned', start_date__gte=now
    ...
    def overdue(self, request):
        ...
        now = timezone.now()
        activities = Activity.objects.filter(
            status='planned', start_date__lt=now
        ).order_by('start_date')
...
class DashboardViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        overdue_activities = Activity.objects.filter(
            status='planned', start_date__lt=today_start
        ).count()

        upcoming_activities = Activity.objects.filter(
            status='planned', start_date__gte=today_start
        ).count()

        # Pipeline by stage
        pipeline_by_stage = (
            Opportunity.objects.filter(~Q(stage__is_won=True) & ~Q(stage__is_lost=True))
        ...
        recent_leads = ContactListSerializer(
            Contact.objects.order_by('-created_at')[:5], many=True
        ).data

        recent_opportunities = OpportunityListSerializer(
            Opportunity.objects.order_by('-created_at')[:5], many=True
        ).data
...
class ChatbotViewSet(viewsets.ViewSet):
    ...
    def create_opportunity(self, request):
        ...
        try:
            ...
            opportunity = Opportunity.objects.create(
                name=data.get(
                    'name',
                    f'Opportunité via chatbot - {timezone.now().strftime("%Y-%m-%d")}',
                ),
                company=company,
                stage=first_stage,
                ...
                description=data.get(
                    'description', 'Créé automatiquement via le chatbot'
                ),
            ...
            return Response(
                {'id': opportunity.id, 'name': opportunity.name, 'status': 'created'},
    ...
    def log_interaction(self, request):
        ...
        try:
            ...
            return Response(
                {'id': activity.id, 'status': 'logged'}, status=status.HTTP_201_CREATED
            )
...
def chatbot_qualify(request):
    ...
    if company_serializer.is_valid():
        ...
        try:
            ...
            return Response(
                {
                    'error': "Impossible de créer ou trouver l'entreprise",
                    'details': company_serializer.errors,
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
                ...
                return Response(
                    {
                        ...
                        'details': contact_serializer.errors,
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
        except Contact.DoesNotExist:
    ...
    try:
        ...
        opportunity = Opportunity.objects.create(
            name=data.get(
                'opportunity_name',
                f'Lead chatbot - {contact.first_name} {contact.last_name}',
            ),
            company=company,
            stage=first_stage,
            description=data.get(
                'opportunity_description', data.get('need', 'Besoin non spécifié')
            ),
            created_by=request.user,
        )
    ...
    try:
        activity_type, _ = ActivityType.objects.get_or_create(
            name='Qualification Chatbot', defaults={'icon': 'form', 'color': '#52c41a'}
        )
...
def chatbot_faq(request):
    ...
    try:
        ...
        activity_type, _ = ActivityType.objects.get_or_create(
            name='FAQ Chatbot', defaults={'icon': 'question-circle', 'color': '#722ed1'}
        )
        ...
        if 'contact_id' in data:
            try:
                contact = Contact.objects.get(id=data['contact_id'])
                activity = Activity.objects.create(
                    **activity_data, created_by=request.user
                )
                activity.contacts.add(contact)
            except Contact.DoesNotExist:
                activity = Activity.objects.create(
    ...
    return Response(
        {'status': 'logged', 'interaction_id': getattr(activity, 'id', None)}
    )
...
def chatbot_appointment(request):
    ...
    try:
        ...
        return Response(
            {'error': 'Contact non trouvé'}, status=status.HTTP_404_NOT_FOUND
        )

    # Récupérer ou créer le type d'activité "Rendez-vous"
    activity_type, _ = ActivityType.objects.get_or_create(
        name='Rendez-vous', defaults={'icon': 'calendar', 'color': '#1890ff'}
    )
    ...
    try:
        ...
        if data.get('reminder', False):
            ...
            activity.reminder_datetime = start_date - timezone.timedelta(
                hours=1
            )  # 1 heure avant par défaut
            activity.save()
...
def chatbot_support(request):
    ...
    activity_type, _ = ActivityType.objects.get_or_create(
        name='Support', defaults={'icon': 'tool', 'color': '#f5222d'}
    )
    ...
    try:
        ...
        return Response(
            {'status': 'success', 'ticket_id': ticket_id, 'activity_id': activity.id},
            status=status.HTTP_201_CREATED,
        )
