...
from ..models import Tax, Account
...
class TaxService:
    ...
    def get_vat_rates():
        ...
        return {
            'standard': {
                ...
                'description': _('Taux normal applicable à la majorité des biens et services.')
            },
            'intermediate1': {
                'rate': 14,
                'label': _('Taux intermédiaire 1 (14%)'),
                'description': _('Applicable à certains produits et services spécifiques.')
            },
            'intermediate2': {
                'rate': 10,
                'label': _('Taux intermédiaire 2 (10%)'),
                'description': _('Applicable notamment au secteur touristique et certains produits alimentaires.')
            },
            'reduced': {
                'rate': 7,
                'label': _('Taux réduit (7%)'),
                'description': _('Applicable aux produits de première nécessité et certains services sociaux.')
            },
            'zero': {
                'rate': 0,
                'label': _('Taux zéro (0%)'),
                'description': _('Applicable aux exportations et certaines opérations exonérées.')
            }
        }
    ...
    def calculate_vat_from_document(document, discount_amount=None):
        ...
        if is_tax_exempt:
            return {
                'vat_amounts': {},
                'total_untaxed': document.total_untaxed if hasattr(document, 'total_untaxed') else Decimal('0.0'),
                'total_tax': Decimal('0.0'),
                'total_amount': document.total_amount if hasattr(document, 'total_amount') else Decimal('0.0'),
                'is_tax_exempt': True
            }
        ...
        if hasattr(document, 'items'):
            for item in document.items.all():
                ...
                vat_amounts[vat_rate] += TaxService.calculate_tax_amount(line_amount, None, False)
        ...
        if discount_amount:
            ...
            if hasattr(document, 'items'):
                total_before_discount = sum(item.quantity * item.unit_price for item in document.items.all())
                discount_ratio = Decimal('1.0')
                
                if total_before_discount > 0:
                    discount_ratio = (total_before_discount - discount_amount) / total_before_discount
    ...
    def is_exempt_operation(operation_type, currency=None):
        ...
        tax_exempt_operations = ['export', 'investment', 'agriculture', 'education', 'health']
    ...
    def get_vat_accounts():
        ...
        try:
            vat_accounts['collected'] = Account.objects.filter(code__startswith='4455', is_active=True).first()
            vat_accounts['deductible'] = Account.objects.filter(code__startswith='3455', is_active=True).first()
            vat_accounts['payable'] = Account.objects.filter(code__startswith='4456', is_active=True).first()
            vat_accounts['credit'] = Account.objects.filter(code__startswith='3456', is_active=True).first()
        except Exception:
            pass
    ...
    def generate_vat_declaration_entry(period_id, user=None):
        ...
        with transaction.atomic():
            ...
            entry = JournalEntry.objects.create(
                ...
                narration=_("Déclaration de TVA pour la période {}").format(period.name),
                is_manual=False,
                created_by=user,
            ...
            if vat_accounts['collected'] and declaration_data['vat_collected']['total'] > 0:
            ...
            if vat_accounts['deductible'] and declaration_data['vat_deductible']['total'] > 0:
                JournalEntryLine.objects.create(
                    entry_id=entry,
                    account_id=vat_accounts['deductible'],
    ...
    def generate_tax_lines(account_lines):
        ...
        for line in account_lines:
            ...
            if line.get('debit', 0) > 0:
                ...
                is_debit = False  # Si la ligne principale est débitée, la taxe est créditée
            elif line.get('credit', 0) > 0:
                base_amount = line.get('credit', 0)
                is_debit = True  # Si la ligne principale est créditée, la taxe est débitée
