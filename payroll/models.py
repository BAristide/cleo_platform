...
class TaxBracket(models.Model):
    ...
    min_amount = models.DecimalField(
        _('Montant minimum'), max_digits=12, decimal_places=2
    )
    max_amount = models.DecimalField(
        _('Montant maximum'), max_digits=12, decimal_places=2, null=True, blank=True
    )
    rate = models.DecimalField(
        _('Taux'), max_digits=5, decimal_places=2, help_text=_('Taux en pourcentage')
    )
    deduction = models.DecimalField(
        _('Somme à déduire'), max_digits=12, decimal_places=2, default=0
    )
    effective_date = models.DateField(_("Date d'effet"))
    end_date = models.DateField(_('Date de fin'), null=True, blank=True)
...
class EmployeePayroll(models.Model):
    ...
    contract_type = models.ForeignKey(
        ContractType, on_delete=models.PROTECT, verbose_name=_('Type de contrat')
    )
    base_salary = models.DecimalField(
        _('Salaire de base'), max_digits=12, decimal_places=2
    )
    hourly_rate = models.DecimalField(
        _('Taux horaire'), max_digits=10, decimal_places=2, null=True, blank=True
    )
    ...
    transport_allowance = models.DecimalField(
        _('Indemnité de transport'), max_digits=10, decimal_places=2, default=0
    )
...
class PayrollRun(models.Model):
    ...
    period = models.ForeignKey(
        PayrollPeriod, on_delete=models.PROTECT, verbose_name=_('Période')
    )
    name = models.CharField(_('Nom'), max_length=100)
    description = models.TextField(_('Description'), blank=True)
    ...
    validated_date = models.DateTimeField(
        _('Date de validation'), null=True, blank=True
    )
...
class PaySlip(models.Model):
    ...
    employee = models.ForeignKey(
        Employee, on_delete=models.PROTECT, verbose_name=_('Employé')
    )
    number = models.CharField(_('Numéro'), max_length=50, unique=True)

    # Période travaillée
    worked_days = models.DecimalField(
        _('Jours travaillés'), max_digits=5, decimal_places=2, default=26
    )
    absence_days = models.DecimalField(
        _("Jours d'absence"), max_digits=5, decimal_places=2, default=0
    )
    paid_leave_days = models.DecimalField(
        _('Jours de congés payés'), max_digits=5, decimal_places=2, default=0
    )
    unpaid_leave_days = models.DecimalField(
        _('Jours de congés sans solde'), max_digits=5, decimal_places=2, default=0
    )

    # Heures supplémentaires
    overtime_25_hours = models.DecimalField(
        _('Heures supp. 25%'), max_digits=5, decimal_places=2, default=0
    )
    overtime_50_hours = models.DecimalField(
        _('Heures supp. 50%'), max_digits=5, decimal_places=2, default=0
    )
    overtime_100_hours = models.DecimalField(
        _('Heures supp. 100%'), max_digits=5, decimal_places=2, default=0
    )
    ...
    gross_salary = models.DecimalField(
        _('Salaire brut'), max_digits=12, decimal_places=2
    )
    taxable_salary = models.DecimalField(
        _('Salaire imposable'), max_digits=12, decimal_places=2
    )
    ...
    payment_reference = models.CharField(
        _('Référence de paiement'), max_length=100, blank=True
    )
...
class PaySlipLine(models.Model):
    ...
    component = models.ForeignKey(
        SalaryComponent, on_delete=models.PROTECT, verbose_name=_('Composant')
    )
    amount = models.DecimalField(_('Montant'), max_digits=12, decimal_places=2)
    base_amount = models.DecimalField(
        _('Montant de base'), max_digits=12, decimal_places=2, null=True, blank=True
    )
    rate = models.DecimalField(
        _('Taux'), max_digits=5, decimal_places=2, null=True, blank=True
    )
    quantity = models.DecimalField(
        _('Quantité'), max_digits=5, decimal_places=2, null=True, blank=True
    )
    is_employer_contribution = models.BooleanField(
        _('Contribution employeur'), default=False
    )
